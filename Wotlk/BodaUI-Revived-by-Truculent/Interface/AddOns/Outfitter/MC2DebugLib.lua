MCAddon.DebugLib =
{
	Version = 1,
	Addon = MCAddon,
	DebugColorCode = MCAddon.AddonColorCode or GREEN_FONT_COLOR_CODE,
}
...
function MCAddon.DebugLib:Initialize()
...
function MCAddon.DebugLib:FindDebugFrame()
	...
	if self.DebugFrame then
		self.Addon:NoteMessage("Found debugging chat frame")
	end
end

function MCAddon.DebugLib:ClearDebugLog()
...
function MCAddon.DebugLib:AddDebugMessage(pPrefix, pMessage, ...)
	if not self.Initialized then
		self:Initialize()
	end
	
	local vMessage = (pPrefix or "")..self.DebugColorCode..string.format("[%s]", self.Addon.AddonName)..FONT_COLOR_CODE_CLOSE..HIGHLIGHT_FONT_COLOR_CODE..pMessage..FONT_COLOR_CODE_CLOSE
...
function MCAddon:DebugMessage(pMessage, ...)
...
function MCAddon:TestMessage(pMessage, ...)
...
function MCAddon:ErrorMessage(pMessage, ...)
...
function MCAddon:NoteMessage(pMessage, ...)
	local vMessage = self.DebugLib:AddDebugMessage("", pMessage, ...)
	DEFAULT_CHAT_FRAME:AddMessage(vMessage)
end

function MCAddon:DebugTable(pValue, pValuePath, pMaxDepth, pMode)
...
function MCAddon:DebugStack(pPrefix, pDepth)
...
function MCAddon:ErrorStack(pPrefix, pDepth)
	local vCallStack = self.DebugLib:GetCallStack(pPrefix, pDepth, 1)
...
function MCAddon:DebugMark()
	self:DebugMessage("————————————————————————————————————————")
end

function MCAddon.DebugLib:ReduceAddonPath(pPath)
	return string.gsub(pPath, "^Interface\\AddOns\\[^\\]*\\", "")
end

function MCAddon.DebugLib:GetCallStack(pPrefix, pDepth, pDepthOffset)
	local vCallStack = {}
...
function MCAddon.DebugLib:NewBuckets(pInterval)
	local vBuckets =
	{
		Interval = pInterval,
...
function MCAddon.DebugLib:BucketsAddSample(pValue, pTime)
...
function MCAddon.DebugLib:BucketsGetValue()
...
function MCAddon.DebugLib:NewPerfMonitor(pLabel)
	local vPerfMonitor =
	{
		Label = pLabel,
...
function MCAddon.DebugLib:PerfMonitorFunctionEnter(pTime)
	self.StartTime = pTime
	self.StartMem = gcinfo()
end

function MCAddon.DebugLib:PerfMonitorFunctionExit(pTime)
...
function MCAddon.DebugLib:PerfMonitorDumpValue()
...
function MCAddon.DebugLib.BoolToString(pValue)
	if pValue then
		return "true"
	else
		return "false"
...
function MCAddon.DebugLib:ShowFrameStatus(pFrame, pPrefix)
...
function MCAddon.DebugLib:ShowParentTree(pFrame, pPrefix)
	if not pPrefix then
		pPrefix = ""
	end
...
function MCAddon.DebugLib:ShowAnchorTree(pFrame, pPrefix)
	if not pPrefix then
		pPrefix = ""
	end
...
function MCAddon.DebugLib:ShowFrameTree(pFrame)
