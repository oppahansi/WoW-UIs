...
function Outfitter:GenerateEquipModule(pModule, pSettings, pExistingScript)
	...
	if type(pModule.GetEquipHeader) == "string" then
		vScript = pModule.GetEquipHeader.."\n"..vScript
	elseif type(pModule.GetEquipHeader) == "function" then
		vScript = pModule:GetEquipHeader(pSettings).."\n"..vScript
	end
	
	if type(pModule.GetEquipFooter) == "string" then
		vScript = vScript.."\n"..pModule.GetEquipFooter
	elseif type(pModule.GetEquipFooter) == "function" then
...
Outfitter.ScriptModules.RogueStealth.GetEquipHeader =
[[
...
Outfitter.ScriptModules.RogueStealth.GetDisableHeader =
[[
...
if setting.Trinkets and #setting.Trinkets > 0 then
    itemInfo0 = Outfitter:FindNextCooldownItem(setting.Trinkets)
end
if setting.Trinkets2 and #setting.Trinkets2 > 0 then
    itemInfo1 = Outfitter:FindNextCooldownItem(setting.Trinkets2)
end

if itemInfo0
and Outfitter:GetInventoryCache():ItemsAreSame(itemInfo0, outfit.Items.Trinket0Slot) then
	itemInfo0 = nil
end

if itemInfo1
and Outfitter:GetInventoryCache():ItemsAreSame(itemInfo1, outfit.Items.Trinket1Slot) then
	itemInfo1 = nil
end
...
[[
-- $SETTING DisableNaxx={type="boolean", label="Disable in Naxxramas", default=true}
			...
			if Outfitter.ScriptInputs then
				...
				for vKey, vValue in pairs(Outfitter.ScriptInputs) do
					...
					elseif type(vValue) == "table" then
						vValue.Type = vValue.Type or vValue.type
