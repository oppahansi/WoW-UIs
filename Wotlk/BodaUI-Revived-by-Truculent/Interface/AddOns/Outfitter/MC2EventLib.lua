...
function MCAddon.EventLib:DispatchEvent(pEventID, ...)
	...
	-- self.Addon:DebugMessage("Dispatching event %s", pEventID)
...
function MCAddon.EventLib:RegisterEvent(pEventID, pFunction, pRefParam, pBlind)
...
function MCAddon.EventLib:EventIsRegistered(pEventID, pFunction, pRefParam)
	local vHandlers = self.Handlers[pEventID]
...
function MCAddon.EventLib:UnregisterEvent(pEventID, pFunction, pRefParam)
	local vIsRegistered, vIndex = self:EventIsRegistered(pEventID, pFunction, pRefParam)
...
function MCAddon.EventLib:UnregisterAllEvents(pFunction, pRefParam)
	for vEventID, vHandlers in pairs(self.Handlers) do
		self:UnregisterEvent(vEventID, pFunction, pRefParam)
	end
end

MCAddon.EventLib.EventFrame.EventLib = MCAddon.EventLib
MCAddon.EventLib.EventFrame:SetScript("OnEvent", function (pFrame, pEvent, ...) pFrame.EventLib:DispatchEvent(pEvent, ...) end)
