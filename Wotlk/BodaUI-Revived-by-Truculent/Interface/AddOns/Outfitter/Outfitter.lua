...
Outfitter.cZoneSpecialIDMap =
{
	...
	[Outfitter.LZ["Naxxramas"]] = {"ArgentDawn", "Naxx"},
...
-- leaving the code in but marking those slots as "false" in
...
function Outfitter:InventoryChanged()
	...
	and not self.WeaponsNeedUpdate
...
function Outfitter:RebuildOutfit(pOutfit)
	...
	local vOutfit = self:GenerateSmartOutfit(
		...
		pOutfit.StatParams,
		function (pNewOutfit)
			pOutfit:SetItems(pNewOutfit:GetItems())
	...
	if vOutfit then
		pOutfit:SetItems(vOutfit:GetItems())
		self:OutfitSettingsChanged(pOutfit)
		self:WearOutfit(pOutfit)
		self:Update(true)
	end
...
function Outfitter:InitializeOutfitMenu(pFrame, pOutfit)
	...
	if UIDROPDOWNMENU_MENU_LEVEL == 1 then
		...
		if pOutfit.StatID then
			local vStatName = self:GetStatIDName(pOutfit.StatID)
		...
		self:AddSubmenuItem(pFrame, self.cRebuildFor, "REBUILD_FOR")
	...
	elseif UIDROPDOWNMENU_MENU_LEVEL == 2 then
		...
		elseif UIDROPDOWNMENU_MENU_VALUE == "REBUILD_FOR" then
			for _, vCategory in ipairs(self.StatCategories) do
				self:AddSubmenuItem(pFrame, vCategory.Name, "REBUILD_"..vCategory.CategoryID)
			end
		end -- elseif
...
function Outfitter:GetPlayerAuraStates()
	...
	while true do
		...
		local vStartIndex, vEndIndex, vTextureName = vTexture:find("([^%\\]*)$")
...
function Outfitter:SynchronizeEM()
	...
	if vNumEMOutfits == 0 then
		if not gOutfitter_Settings.PreservedEMOutfits then
			gOutfitter_Settings.PreservedEMOutfits = {}
		end
		
		for vCategoryID, vOutfits in pairs(gOutfitter_Settings.Outfits) do
			for vIndex, vOutfit in ipairs(vOutfits) do
				if vOutfit.StoredInEM then
					gOutfitter_Settings.PreservedEMOutfits[vOutfit.Name] = vOutfit
				end
			end
		end
	end
	...
	for vIndex = 1, vNumEMOutfits do
		...
		if not vOutfit
		and gOutfitter_Settings.PreservedEMOutfits then
			vOutfit = gOutfitter_Settings.PreservedEMOutfits[vName]
	...
	if vNumEMOutfits > 0 then
		gOutfitter_Settings.PreservedEMOutfits = nil
	end
...
function Outfitter:CanEquipBagItem(pBagIndex, pBagSlotIndex)
	...
	return not vItemMinLevel or UnitLevel("player") >= vItemMinLevel
end
...
function Outfitter.NameOutfit_Open(pOutfit)
	...
	if gOutfitter_OutfitToRename then
		...
		Outfitter.DropDown_SetSelectedValue(OutfitterNameOutfitDialogCreateUsing, 0)
...
function Outfitter.NameOutfit_Done()
	...
	and vName ~= "" then
		if gOutfitter_OutfitToRename then
			...
			gOutfitter_OutfitToRename:SetName(vName)
			Outfitter.DisplayIsDirty = true
			...
			local vStat = UIDropDownMenu_GetSelectedValue(OutfitterNameOutfitDialogCreateUsing)
			...
			elseif vStat == "EMPTY" then
				vOutfit = Outfitter:NewEmptyOutfit(vName)
			else
				vOutfit = Outfitter:GenerateSmartOutfit(vName, vStat, Outfitter:GetInventoryCache(), false, nil, function (pOutfit)
					Outfitter.NameOutfit_AddOutfit(pOutfit, vScriptID)
				end)
			...
			Outfitter.NameOutfit_AddOutfit(vOutfit, vScriptID)
		end
	end
...
function Outfitter.NameOutfit_AddOutfit(pOutfit, pScriptID)
	-- Add the outfit
...
function Outfitter.NameOutfit_Update(pCheckForStatOutfit)
	...
	and pCheckForStatOutfit then
		...
		and vStat ~= 0
		and vStat ~= "EMPTY"
		and not vStat.CompareOutfits then -- Don't attempt to test for iterative outfits
...
function Outfitter.OutfitMenuActions.DEPOSITOTHERS(pOutfit)
	Outfitter:DepositOtherOutfits(pOutfit)
end
...
function Outfitter.OutfitMenuActions.WITHDRAWOTHERS(pOutfit)
	Outfitter:WithdrawOtherOutfits(pOutfit)
end
...
function Outfitter:CheckDatabase()
	...
	if gOutfitter_Settings.Version < 18 then
		if gOutfitter_Settings.Outfits then
			for vCategoryID, vOutfits in pairs(gOutfitter_Settings.Outfits) do
				for vIndex, vOutfit in ipairs(vOutfits) do
		...
		gOutfitter_Settings.Version = 18
	end
...
function Outfitter:GetItemUseDuration(pInventorySlot)
	...
	if not OutfitterTooltip:SetInventoryItem("player", self.cSlotIDs[pInventorySlot]) then
		OutfitterTooltip:Hide()
		return false
	end
...
function Outfitter:InventoryItemIsActive(pInventorySlot)
	...
	local vItemCode = self:ParseItemLink(vItemLink)
	local vUseDuration
	
	if self.cItemUseDuration[vItemCode] then
		vUseDuration  = self.cItemUseDuration[vItemCode]
	else
		vUseDuration = self:GetItemUseDuration(pInventorySlot)
...
function Outfitter:SummonCompanionByName(pName, pDelay)
	...
	for vIndex = 1, vNumCompanions do
		local _, vName = GetCompanionInfo("CRITTER", vIndex)
