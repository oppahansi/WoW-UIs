...
MCAddon.SchedulerLib.EventFrame.SchedulerLib = MCAddon.SchedulerLib
MCAddon.SchedulerLib.EventFrame:SetScript("OnUpdate", function (self, elapsed) self.SchedulerLib:OnUpdate(elapsed) end)

function MCAddon.SchedulerLib:ScheduleTask(pDelay, pFunction, pParam, pTaskName)
...
function MCAddon.SchedulerLib:RescheduleTask(pDelay, pFunction, pParam, pTaskName)
	self:UnscheduleTask(pFunction, pParam)
	self:ScheduleTask(pDelay, pFunction, pParam, pTaskName)
end

function MCAddon.SchedulerLib:ScheduleUniqueTask(pDelay, pFunction, pParam, pTaskName)
...
function MCAddon.SchedulerLib:ScheduleRepeatingTask(pInterval, pFunction, pParam, pInitialDelay, pTaskName)
	local vTask =
	{
		Interval = pInterval,
		Function = pFunction,
...
function MCAddon.SchedulerLib:ScheduleUniqueRepeatingTask(pInterval, pFunction, pParam, pInitialDelay, pTaskName)
	if self:FindTask(pFunction, pParam) then
		return
	end
...
function MCAddon.SchedulerLib:SetTaskInterval(pInterval, pFunction, pParam)
...
function MCAddon.SchedulerLib:SetTaskDelay(pDelay, pFunction, pParam)
	local vTask, vIndex = self:FindTask(pFunction, pParam)
...
function MCAddon.SchedulerLib:InsertTask(pTask)
	if not pTask then
...
function MCAddon.SchedulerLib:UnscheduleAllTasks(pFunction, pParam)
	while self:UnscheduleTask(pFunction, pParam) do
	end
end

function MCAddon.SchedulerLib:UnscheduleTask(pFunction, pParam)
	local vTask, vIndex = self:FindTask(pFunction, pParam)
...
function MCAddon.SchedulerLib:FindTask(pFunction, pParam)
...
function MCAddon.SchedulerLib:OnUpdate(pElapsed)
...
function MCAddon.SchedulerLib:OnUpdate2(pElapsed)
	local vTime = GetTime()
	...
	while self.IteratorIndex <= #self.Tasks do
		...
		if not vTask then
			self.Addon:ErrorMessage("nil task at index "..self.IteratorIndex)
		...
		elseif vTask.DidRun then
			...
			-- self.Addon:DebugMessage("Calling task %s", vTask.Name or "anonymous")
...
function MCAddon.SchedulerLib.CompareTasks(pTask1, pTask2)
