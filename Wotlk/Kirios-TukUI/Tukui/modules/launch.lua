	...
	if (TukuiCF.chat.enable == true) and (not IsAddOnLoaded("Prat") or not IsAddOnLoaded("Chatter")) then					
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			if i == 1 then
				...
				frame:SetSize(ChatLeft:GetWidth() - 6, ChatLeft:GetHeight() - 6)
				frame:SetPoint("CENTER", ChatLeft, "CENTER", 0, 0)
			elseif i == 4 then
				frame:ClearAllPoints()
				frame:SetSize(ChatRight:GetWidth() - 6, ChatRight:GetHeight() - 6)
				frame:SetPoint("CENTER", ChatRight, "CENTER", 0, 0)
			end
			...
			if i == 3 then FCF_SetWindowName(frame, "Loots") end
			if i == 4 then FCF_SetWindowName(frame, "Global") end
		end
		...
		ChatFrame_AddChannel(ChatFrame4, tukuilocal.chat_general) -- erf, it seem we need to localize this now
		ChatFrame_AddChannel(ChatFrame4, tukuilocal.chat_defense) -- erf, it seem we need to localize this now
		ChatFrame_AddChannel(ChatFrame4, tukuilocal.chat_recrutment) -- erf, it seem we need to localize this now
		ChatFrame_AddChannel(ChatFrame4, tukuilocal.chat_lfg) -- erf, it seem we need to localize this now
...
TukuiOnLogon:SetScript("OnEvent", function(self, event)
	...
	ChatFrame1:AddMessage(tukuilocal.login..name.."!", 0, .7, 1)
end)
