...
function HydraDB.SetBorder(f)
	...
	border:SetBackdrop { edgeFile = TukuiCF["media"].blank, edgeSize = 3, insets = {left = 0, right = 0, top = 0, bottom = 0} }
...
do
	...
	TukuiDB.PostNamePosition = function(self)
		...
		--	self.Power.value:SetAlpha(0)
			--self.Name:SetPoint("LEFT", self.panel, "LEFT", 4, 30)
		--end
	end
	...
	local FormatTime = function(s)
		...
		--return format("%.1f", s)
		return floor(s + 0.5), (s * 100 - floor(s * 100))/100 -- YAN BUFF/DEBUFF TIMER TRESHOLD
	end
		...
		if self.timeLeft then
			...
			if self.elapsed >= 0.1 then
				...
				if self.timeLeft > 0 then
					...
					if self.timeLeft <= 2 then -- YAN
						self.remaining:SetTextColor(0.99, 0.31, 0.31)
						self.remaining:SetFont(TukuiCF.media.minimal, 14, "OUTLINEMONOCHROME")
					else
						self.remaining:SetTextColor(1, 1, 1)
						self.remaining:SetFont(TukuiCF.media.minimal, 10, "OUTLINEMONOCHROME")
					end
				else
					self.remaining:Hide()
	...
	function TukuiDB.PostCreateAura(element, button)
		...
		button.remaining = TukuiDB.SetFontString(button, TukuiCF["media"].minimal, TukuiCF["unitframes"].auratextscale, "OUTLINE")
