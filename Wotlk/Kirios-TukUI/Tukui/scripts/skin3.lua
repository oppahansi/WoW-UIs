...
local function SetModifiedBackdrop(self)
	self:SetBackdropColor(.075,.075,.075,1)
	self:SetBackdropBorderColor(0, .8, 1)
end
	...
	if addon == "Tukui" then
		...
		for i = 1, getn(skins) do
			...
			--if _G[skins[i]] ~= _G["GhostFrameContentsFrame"] and _G[skins[i]] ~= _G["AutoCompleteBox"] and _G[skins[i]] ~= _G["BNToastFrame"] then -- frame to blacklist from create shadow function
				...
				--_G[skins[i]]:SetBackdropColor(.075,.075,.075,.7)
				--_G[skins[i]]:SetBackdropBorderColor(unpack(TukuiCF["media"].bordercolor))
			--	HydraDB.SetBorder(_G[skins[i]])
			--end
		end
		...
		_G["ReadyCheckFrame"]:HookScript("OnShow", function(self) if UnitIsUnit("player", self.initiator) then self:Hide() GhostFrame:Hide() end end) -- bug fix, don't show it if initiator
		_G["GhostFrameContentsFrame"]:SetWidth(TukuiDB.Scale(TukuiMinimap:GetWidth() + TukuiDB.Scale(4)))
		_G["GhostFrameContentsFrame"]:ClearAllPoints()
		_G["GhostFrameContentsFrame"]:SetPoint("TOP", TukuiMinimap, "BOTTOM", 0, TukuiDB.Scale(-6))
		_G["GhostFrameContentsFrame"].SetPoint = TukuiDB.dummy
		--TukuiDB.CreateShadow(_G["GhostFrameContentsFrame"])
		--TukuiDB.CreateShadow(_G["GhostFrame"])
		_G["GhostFrame"]:SetFrameStrata("HIGH")
		_G["GhostFrame"]:SetFrameLevel(10)
		_G["GhostFrame"]:ClearAllPoints()
		_G["GhostFrame"]:SetPoint("TOP", TukuiMinimap, "BOTTOM", 0, TukuiDB.Scale(-6))
