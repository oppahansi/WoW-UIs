...
hooksecurefunc("GameTooltip_SetDefaultAnchor", function(self, parent)
	if db.cursor == true then
		...
		if InCombatLockdown() and db.hidecombat == true then
			...
			elseif ChatRight:IsShown() or ChatRight:IsVisible() then
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT",  TukuiTabRight, "TOPRIGHT", 0, TukuiDB.Scale(3))
			else
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT",  TukuiInfoRight, "TOPRIGHT", 0, TukuiDB.Scale(3))
			end
		end
	end
end)
...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
	...
	if not self.text then
		...
		self.text:SetFont(TukuiCF.media.minimal, 10, "OUTLINE")
		self.text:Show()
		if unit then
