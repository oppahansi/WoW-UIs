...
local function SetModifiedBackdrop(self)
	self:SetBackdropColor(.055,.055,.055,1)
...
TukuiSkin:SetScript("OnEvent", function(self, event, addon)
	...
	if addon == "Tukui" then
		...
		for i = 1, getn(skins) do
			...
			if _G[skins[i]] ~= _G[skins[i]] ~= _G["AutoCompleteBox"] and _G[skins[i]] ~= _G["BNToastFrame"] then -- frame to blacklist from create shadow function
