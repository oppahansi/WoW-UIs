...
local function SetChatStyle(frame)
	...
	for i = 1, NUM_CHAT_WINDOWS do
		if i == 4 then
			...
			--_G["ChatFrame"..i]:SetFont(TukuiCF.media.minimal, 10, "OUTLINE")
			---tab:GetFontString():SetTextFont(TukuiCF.media.minimal, 10, "OUTLINE")
		end
	end
	
	-- mouse-over color tabs YAN
    _G[chat.."Tab"]:HookScript("OnShow", function(self) self:GetFontString():SetFont(TukuiCF.media.minimal, 10, "OUTLINE") end) --YAN
	_G[chat.."Tab"]:HookScript("OnEnter", function(self) self:GetFontString():SetTextColor(unpack(TukuiCF.chat.tabmouseover)) end)
	_G[chat.."Tab"]:HookScript("OnLeave", function(self) self:GetFontString():SetTextColor(unpack(Ctabcolor)) end)

	-- :)
	GeneralDockManager:SetParent(ChatLeft)
	...
	-- this is since I have animated chat windows
	_G[chat.."EditBox"]:SetParent(TukuiInfoLeft);
	...
	if TukuiCF.chat.editboxanimation == true then
		AnimGroup(_G[chat.."EditBox"], 0, TukuiDB.Scale(-20), TukuiCF.chat.editspeed) -- want a fast transition

		_G[chat.."EditBox"]:HookScript("OnShow", function(self, ...)
			SlideIn(_G[chat.."EditBox"])
			
			-- do we want to auto-show chats with editbox?
			if TukuiCF.chat.autoshow == true then
		...
		_G[chat.."EditBox"]:HookScript("OnEditFocusLost", function(self, ...)
			SlideOut(_G[chat.."EditBox"])
			_G[chat.."EditBox"]:SetAlpha(1)
		end)
	else
	...
	if _G[chat] == _G["ChatFrame2"] then
		FCF_SetWindowName(_G[chat], "Combat Log")
	end
...
local function SetupChatPosAndFont(self)	
	for i = 1, NUM_CHAT_WINDOWS do
		...
		if i == 1 then
			chat:ClearAllPoints()
			chat:SetParent(ChatLeft)
			chat:SetFrameStrata("MEDIUM")
			chat:SetSize(ChatLeft:GetWidth() - 6, ChatLeft:GetHeight() - 6)
			chat:SetPoint("CENTER", ChatLeft, "CENTER", 0, 0)
			FCF_SavePositionAndDimensions(chat)
			tab:SetParent(ChatLeft)
		
		-- setting parent for my animation, don't touch
		elseif i == 2 then
		...
		elseif i == 4 then
			if not chat.isDocked then
				chat:ClearAllPoints()
				chat:SetParent(ChatRight)
				chat:SetFrameStrata("MEDIUM")
				chat:SetSize(ChatRight:GetWidth() - 6, ChatRight:GetHeight() - 6)
				chat:SetPoint("CENTER", ChatRight, "CENTER", 0, 0)
				FCF_SavePositionAndDimensions(chat)
				tab:SetParent(ChatRight)
			end
		end
	end
end

-- Rewritten toast frame
BNToastFrame:HookScript("OnShow", function(self, ...)
	if ChatLeft:IsShown() then
		self:ClearAllPoints()
		self:SetPoint("BOTTOMLEFT",  TukuiTabLeft, "TOPLEFT", 0, TukuiDB.Scale(3))
	else
		self:ClearAllPoints()
		self:SetPoint("BOTTOMLEFT",  TukuiInfoLeft, "TOPLEFT", 0, TukuiDB.Scale(3))
	end
	TukuiDB.CreateShadow(BNToastFrame)
end)
...
function TukuiDB.ChatCopyButtons()
	for i = 1, NUM_CHAT_WINDOWS do
		...
		button:SetPoint("TOPRIGHT", TukuiDB.Scale(-1), TukuiDB.Scale(-1))
		...
		buttontext:SetFont(TukuiCF.media.font,13,"OUTLINE")
