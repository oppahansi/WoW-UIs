...
local BORDER_TEXTURE = "Interface\\AddOns\\Stuffing\\border"
...
local function SetDefaults()
	StuffingDB.cols = 10				-- columns to use for bags.
	StuffingDB.bankCols = 15			-- columns to use for bank.
	StuffingDB.bsize = 37				-- height and width of the item buttons (default: 37).
	StuffingDB.spacing = 4				-- spacing between buttons.
	StuffingDB.padding = 10				-- left, right and bottom padding.
	StuffingDB.clamp = 1				-- clamp bag window to screen.
	StuffingDB.rarity_glow = 1			-- color item borders by rarity.
	StuffingDB.search_glow = {0.8, 0.8, 0.3}	-- glow color used for search highlighting.
	StuffingDB.locked = 0				-- lock bag frame position.
	StuffingDB.font = "Fonts\\FRIZQT__.TTF"
	StuffingDB.fontSize = 12
	StuffingDB.background = "Interface\\ChatFrame\\ChatFrameBackground"
	StuffingDB.edge = "Interface\\Tooltips\\UI-Tooltip-Border"
	StuffingDB.edgeSize = 16
	StuffingDB.backdrop_color = {0.1, 0.1, 0.1, 1}
	StuffingDB.border_color = {0, 0, 0, 1}
	StuffingDB.quest_glow = 1
	StuffingDB.quest_glow_color = {1.0, 0.3, 0.3}
	StuffingDB.custom_skin = 0
	StuffingDB.hide_soulbag = 0

	StuffingDB.force_defaults = 0

	StuffingDB.soulbag_color = {0.8, 0.2, 0.2, 1}
	StuffingDB.special_color = {0.2, 0.2, 0.8, 1}
	StuffingDB.quiver_color = {0.8, 0.8, 0.2, 1}

	StuffingDB.bag_bars = 1

	StuffingDB.custom_stackfont = 0
	StuffingDB.stackfont = "Fonts\\FRIZQT__.TTF"
	StuffingDB.stackfont_size = 8
end
...
local function Stuffing_OpenConfig()
	local l, r = LoadAddOn ("Stuffing_Config")
	if l == nil then
		Print(string.format(L["Can't open config: %s"], _G["ADDON_" .. r]))
		return
	end
...
function Stuffing:SlotUpdate(b)
	...
	if(clink) then
		...
		if StuffingDB.quest_glow == 1 then
	...
	if b.Glow then
		...
		if b.rarity then
			if b.rarity > 1 then
				...
				b.Glow:Show()
			elseif b.qitem and StuffingDB.quest_glow == 1 then
				b.Glow:SetVertexColor(unpack(StuffingDB.quest_glow_color))
				b.Glow:Show()
			end
		end
	end
...
function Stuffing:BagFrameSlotNew (slot, p)
	...
	if slot > 3 then
		...
		--[[
		local is = ret.frame:GetInventorySlot()
		local texture = GetInventoryItemTexture("player", is)

		if texture == nil then
			texture = "interface\\paperdoll\\UI-PaperDoll-Slot-Bag.blp"
		end

		SetItemButtonTexture(ret.frame, texture)
		ret.frame:Update()
		--]]
...
function Stuffing:SlotNew (bag, slot)
	...
	if StuffingDB.rarity_glow == 1 and not ret.Glow then
	...
	for _, b in ipairs(self.buttons) do
		if b.name then
			if not string.find (string.lower(b.name), str) then
				...
				if b.Glow then
					b.Glow:Show()
					b.Glow:SetVertexColor(unpack(StuffingDB.search_glow))
				end
			end
		end
	end
...
function Stuffing.Menu(self, level)
	...
	info.checked = function()
		return StuffingDB.bag_bars == 1
	end

	info.func = function()
		if StuffingDB.bag_bars == 1 then
			StuffingDB.bag_bars = 0
		else
			StuffingDB.bag_bars = 1
		end
		Stuffing:Layout()
...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		...
		if StuffingDB.locked == 0 then
			f:SetScript("OnMouseDown", StartMoving)
			f:SetScript("OnMouseUp", StopMoving)
		end
	end

	local x = StuffingDB[n .. "PosX"] or 0
	local y = StuffingDB[n .. "PosY"] or 0
...
function Stuffing:InitBags()
	...
	detail:SetPoint("TOPLEFT", f, StuffingDB.padding, -10)
	...
	f:SetScript("OnEvent", function (self, e)
		self.gold:SetText (GetCoinTextureString(GetMoney(), StuffingDB.fontSize))
	end)
...
function Stuffing:Layout(lb)
	...
	if lb then
		bs = BAGS_BANK
		cols = StuffingDB.bankCols
		f = self.bankFrame
	else
		bs = BAGS_BACKPACK
		cols = StuffingDB.cols
		f = self.frame

		f.gold:SetText (GetCoinTextureString(GetMoney(), StuffingDB.fontSize))
		f.editbox:SetFont(StuffingDB.font, StuffingDB.fontSize)
		f.detail:SetFont(StuffingDB.font, StuffingDB.fontSize)
		f.gold:SetFont(StuffingDB.font, StuffingDB.fontSize)

		f.detail:ClearAllPoints()
		f.detail:SetPoint("TOPLEFT", f, StuffingDB.padding, -10)
		f.detail:SetPoint("RIGHT", -(16 + 24), 0)
	end

	f:SetClampedToScreen(StuffingDB.clamp)
	f:SetBackdrop({
		bgFile = StuffingDB.background,
		edgeFile = StuffingDB.edge,
		edgeSize = StuffingDB.edgeSize,
		insets = {left = 3, right = 3, top = 3, bottom = 3}
	})
	f:SetBackdropColor(unpack(StuffingDB.backdrop_color))
	f:SetBackdropBorderColor(unpack(StuffingDB.border_color))


	-- bag frame stuff
	local fb = f.bags_frame
	if StuffingDB.bag_bars == 1 then
		fb:SetClampedToScreen(StuffingDB.clamp)
		fb:SetBackdrop({
			bgFile = StuffingDB.background,
			edgeFile = StuffingDB.edge,
			edgeSize = StuffingDB.edgeSize,
		...
		fb:SetBackdropColor(unpack(StuffingDB.backdrop_color))
		fb:SetBackdropBorderColor(unpack(StuffingDB.border_color))

		local bsize = 30
		if lb then bsize = 37 end

		local w = 2 * StuffingDB.padding
		w = w + ((#bs - 1) * bsize)
		w = w + (StuffingDB.spacing * (#bs - 2))

		fb:SetHeight(2 * StuffingDB.padding + bsize)
		fb:SetWidth(w)
		fb:Show()
	else
		fb:Hide()
	...
	for _, v in ipairs(bs) do
		if (not lb and v <= 3 ) or (lb and v ~= -1) then
			...
			local xoff = StuffingDB.padding

			xoff = xoff + (idx * bsize) -- StuffingDB.bsize)
			xoff = xoff + (idx * StuffingDB.spacing)
	...
	for _, i in ipairs(bs) do
		...
		if x > 0 then
			...
			if not (StuffingDB.hide_soulbag == 1 and self.bags[i].bagType == ST_SOULBAG) then
	...
	f:SetWidth(cols * StuffingDB.bsize
			+ (cols - 1) * StuffingDB.spacing
			+ StuffingDB.padding * 2)

	f:SetHeight(rows * StuffingDB.bsize
			+ (rows - 1) * StuffingDB.spacing
			+ off + StuffingDB.padding * 2)
	...
	for _, i in ipairs(bs) do
		...
		if bag_cnt > 0 then
			...
			if not (StuffingDB.hide_soulbag == 1 and bagType == ST_SOULBAG) then
				self.bags[i]:Show()
				...
				for j = 1, bag_cnt do
					...
					xoff = StuffingDB.padding + (x * StuffingDB.bsize)
							+ (x * StuffingDB.spacing)

					yoff = off + StuffingDB.padding + (y * StuffingDB.bsize)
							+ ((y - 1) * StuffingDB.spacing)
					yoff = yoff * -1

					b.frame:ClearAllPoints()
					b.frame:SetPoint("TOPLEFT", f, "TOPLEFT", xoff, yoff)
					b.frame:SetHeight(StuffingDB.bsize)
					b.frame:SetWidth(StuffingDB.bsize)
					b.frame:Show()

					local normalTex = _G[b.frame:GetName() .. "NormalTexture"]
					if StuffingDB.custom_skin == 1 then
						normalTex:SetTexture(BORDER_TEXTURE)
						normalTex:ClearAllPoints()
						normalTex:SetAllPoints(b.frame)
					end
					normalTex:SetHeight(StuffingDB.bsize / 37 * 64)
					normalTex:SetWidth(StuffingDB.bsize / 37 * 64)
					normalTex:Show()
					b.normalTex = normalTex

					if bagType == ST_QUIVER then
						normalTex:SetVertexColor(unpack(StuffingDB.quiver_color))
					elseif bagType == ST_SOULBAG then
						normalTex:SetVertexColor(unpack(StuffingDB.soulbag_color))
					elseif bagType == ST_SPECIAL then
						normalTex:SetVertexColor(unpack(StuffingDB.special_color))
					...
					if b.Glow then
						b.Glow:SetWidth(StuffingDB.bsize / 37 * 65)
						b.Glow:SetHeight(StuffingDB.bsize / 37 * 65)
					end

					if StuffingDB.custom_stackfont == 1 then
						local scount = _G[b.frame:GetName() .. "Count"]
						--scount:GetFontObject():SetFont (StuffingDB.stackfont, StuffingDB.stackfont_size, "OUTLINE")
						scount:SetFont (StuffingDB.stackfont, StuffingDB.stackfont_size, "OUTLINE")
						b.scount = scount
					end
...
function Stuffing:ADDON_LOADED(addon)
	if addon ~= "Stuffing" then
		return nil
	end

	-- new options
	if not StuffingDB.font then
		StuffingDB.font = "Fonts\\FRIZQT__.TTF"
	end

	if not StuffingDB.fontSize then
		StuffingDB.fontSize = 14
	end

	if not StuffingDB.quest_glow then
		StuffingDB.quest_glow = 1
	end

	if not StuffingDB.quest_glow_color then
		StuffingDB.quest_glow_color = {0.3, 0.3, 0.8}
	end

	if not StuffingDB.custom_skin then
		StuffingDB.custom_skin = 0
	end

	if not StuffingDB.hide_soulbag then
		StuffingDB.hide_soulbag = 0
	end

	if StuffingDB.force_defaults then
		if StuffingDB.force_defaults == 1 then
			SetDefaults()
			StuffingDB.force_defaults = 1
		end
	end

	if not StuffingDB.bag_bars then
		StuffingDB.bag_bars = 0
	end

	if not StuffingDB.soulbag_color then
		StuffingDB.soulbag_color = {0.8, 0.2, 0.2, 1}
		StuffingDB.special_color = {0.2, 0.2, 0.8, 1}
		StuffingDB.quiver_color = {0.8, 0.8, 0.2, 1}
	end

	if not StuffingDB.stackfont then
		StuffingDB.custom_stackfont = 0
		StuffingDB.stackfont = "Fonts\\FRIZQT__.TTF"
		StuffingDB.stackfont_size = 8
	end
	...
	if id > 28 then
		for _, v in ipairs(self.bagframe_buttons) do
			...
			if v.frame and v.frame.GetInventorySlot then
				...
				--[[
				local is = v.frame:GetInventorySlot()
				texture = GetInventoryItemTexture("player", is)
				--]]
				--print(texture)
			...
			--SetItemButtonTexture(v.frame, texture)
		end
	end
