...
--------------------------------------------------------------------------------------------------------
--                                   Gather Unit Details & Modify Tip                                 --
--------------------------------------------------------------------------------------------------------
...
local function ModifyUnitTooltip()
	...
	-- Info Line
	local lineIndex = 2 + (GetCVar("colorblindMode") == "1" and UnitIsVisible(unit) and 1 or 0) + (hasGuildTitle and 1 or 0);
...
local function SetupAuras()
	...
	if (cfg.showBuffs) then
		while (true) do
			...
			if (not iconTexture) or (index / maxAuras > cfg.auraMaxRows) then
				break;
			end
			if (not cfg.selfAurasOnly or casterUnit == "player" or casterUnit == "pet" or casterUnit == "vehicle") then
				local aura = auras[index] or CreateAura();
				-- Anchor It
				aura:ClearAllPoints();
				if (index == 1) then
				...
				elseif (maxAuras == 1) or (index % maxAuras == 1) then
					if (cfg.aurasAtBottom) then
	...
	if (cfg.showDebuffs) and (index / maxAuras <= cfg.auraMaxRows) then
		local buffCount = (index - 1);
		local debuffIndex = 1;
		local lineOffset = floor(buffCount / maxAuras);
		local buffsOnFirstLine = (maxAuras - buffCount % maxAuras);
		while (true) do
			local _, _, iconTexture, count, debuffType, duration, endTime, casterUnit = UnitAura(u.token,debuffIndex,"HARMFUL");
			if (not iconTexture) or (index / maxAuras > cfg.auraMaxRows) then
				break;
			end
			if (not cfg.selfAurasOnly or casterUnit == "player" or casterUnit == "pet" or casterUnit == "vehicle") then
				...
				elseif ((index - 1) % maxAuras == 0) then
					if (cfg.aurasAtBottom) then
						aura:SetPoint("TOPRIGHT",gtt,"BOTTOMRIGHT",0,(cfg.auraSize + 1) * floor(index / maxAuras) * -1);
					else
						aura:SetPoint("BOTTOMRIGHT",gtt,"TOPRIGHT",0,(cfg.auraSize + 1) * floor(index / maxAuras));
					end
				else
...
-- Replace GameTooltip_SetDefaultAnchor (For Re-Anchoring)
...
gtt.Show = function(self,...)
	gttShow(self,...);
	if (bars.offset) then
		gtt_newHeight = (gtt:GetHeight() + bars.offset);
		gtt:SetHeight(gtt_newHeight);
	end
end
...
gtt.FadeOut = function(self,...)
	if (not u.token) or (not cfg.overrideFade) then
		gtt.fadeOut = 2; -- Don't allow the OnUpdate handler to run the fadeout/update code
		gttFadeOut(self,...);
...
local function GTTHook_OnShow(self,...)
	gtt_anchorType, gtt_anchorPoint = GetAnchorPosition();
	if (gtt_anchorType == "mouse") and (gtt:GetAnchorType() ~= "ANCHOR_CURSOR") then
		tt:AnchorFrameToMouse(self);
	end
	if (self:IsOwned(UIParent)) and (not gtt:GetUnit()) then
...
local function GTTHook_OnUpdate(self,elapsed)
	...
	if (gtt:GetAnchorType() == "ANCHOR_CURSOR") then
		gtt:SetBackdropColor(unpack(cfg.tipColor));
		gtt:SetBackdropBorderColor(unpack(cfg.tipBorderColor));
		return;
	-- Anchor GTT to Mouse
	...
	if (gtt_newHeight) and (abs(gtt:GetHeight() - gtt_newHeight) > 0.1) then
		gtt:SetHeight(gtt_newHeight);
	end
end
...
local function GTTHook_OnTooltipSetUnit(self,...)
	if (cfg.hideAllTipsInCombat or cfg.hideUFTipsInCombat and gtt:GetOwner() ~= UIParent) and (UnitAffectingCombat("player")) then
		gtt:Hide();
		return;
	end
	...
	u.token = unit;
	gtt.fadeOut = nil; -- Az: Sometimes this wasn't getting reset, the fact a cleanup isn't performed at this point, now that it was moved to "OnTooltipCleared" is very bad, so this is a fix
...
local function ApplyTipTacAppearance(first)
	...
	if (first) and (not u.isPlayer) then
		u.title = GameTooltipTextLeft2:GetText();
