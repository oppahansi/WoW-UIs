local revision = tonumber(string.sub("$Revision: 73803 $", 12, -3)) or 1
...
function EBB_Bar.prototype:OnClick(button)
	...
	elseif button == "RightButton" then
		...
		if self.data.realtype == "BUFF" or self.data.realtype == "DEBUFF" then
			CancelPlayerBuff(self.data.id)
...
function EBB_Bar.prototype:OnEnter(this)
	...
	if self.parent.layout.target == "player" then
		...
		if realtype == "BUFF" then
			GameTooltip:SetUnitBuff(self.parent.layout.target, self.data.id)
		elseif realtype == "DEBUFF" then
			GameTooltip:SetUnitDebuff(self.parent.layout.target, self.data.id)
		end
	end
end
...
function EBB_Bar.prototype:OnUpdate(elapsed)
	...
	local total_elapsed = self.total_elapsed + elapsed
	local fraction = (data.timeleft - total_elapsed) / data.timemax

	if fraction > 1 then
		ElkBuffBars:Print(string.format("|cffff0000[EBB] ERROR:|r %d - %s - %d / %d", data.id, data.name, data.timeleft, data.timemax))
		fraction = 0
	end
	...
	if barwidth + .2 < self.barwidth then -- this could be a visual change
		if barwidth < 1 then
			...
			if layout.barright then
				frames.bar:SetTexCoord(fraction, 1, 0, 1)
			else
...
-- container
	if not frames.container then
		...
		frames.container:SetScript("OnEnter", function(this) this.bar:OnEnter(this) end )
...
function EBB_Bar.prototype:UpdateData(data)
	...
	if layout.bar then
		if data.untilcancelled then
			...
			local fraction = data.timeleft / data.timemax
			...
			if data.id >= 0 then -- no scripts for Blessing of Demonstration
				frames.container:SetScript("OnUpdate", function(this, elapsed) this.bar:OnUpdate(elapsed) end)
			end
		end
		local color = layout["barcolor"]
		if data.type == "DEBUFF" and layout.debufftypecolor then
			...
			color = {debuffcolor.r, debuffcolor.g, debuffcolor.b, color[4]}
		end
		frames.bar:SetVertexColor(unpack(color))
	else
		frames.container:SetScript("OnUpdate", nil)
	end
	if layout.bgbar then 
...
function EBB_Bar.prototype:GetDataString(datatype)
	...
	if datatype == "TIMELEFT" then return self:GetTimeString(self.data.timeleft, self.layout.timeformat) end
...
function EBB_Bar.prototype:UpdateTimeleft()
	...
	-- new data.timeleft was set, reset time elapsed since last setting
	self.total_elapsed = 0
	
	if layout.textTL == "TIMELEFT" then
		frames.textTL:SetText(self:GetDataString("TIMELEFT"))
	end
	if layout.textTR == "TIMELEFT" then
		frames.textTR:SetText(self:GetDataString("TIMELEFT"))
	end
	if layout.textTL and layout.textBL == "TIMELEFT" then
		frames.textBL:SetText(self:GetDataString("TIMELEFT"))
	end
	if layout.textTL and layout.textBR == "TIMELEFT" then
		frames.textBR:SetText(self:GetDataString("TIMELEFT"))
	end
	self:UpdateTextWidth()
end
