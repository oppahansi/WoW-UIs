local revision = tonumber(string.sub("$Revision: 70705 $", 12, -3)) or 1
...
local sorting = {
	...
	timeleft = function(a, b)
			...
			if a.timeleft < b.timeleft then
				return false
			elseif a.timeleft > b.timeleft then
				return true
			else
				return a.name < b.name
			end
...
function EBB_BarGroup.prototype:CheckFilter(data)
	...
	if	   (not filter.type[data.type])																								-- type
		...
		or (filter.untilcancelled == "only" and not data.untilcancelled)															-- only untilcancelled
		or (filter.untilcancelled == "hide" and data.untilcancelled)																-- hide untilcancelled
