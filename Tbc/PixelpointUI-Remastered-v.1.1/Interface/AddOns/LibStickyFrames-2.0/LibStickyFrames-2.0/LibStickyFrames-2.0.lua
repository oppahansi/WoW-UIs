...
do
	local function onShow(self)
		UpdateFrameColor(self:GetParent())
	end
	...
	local function onDragStart(self)
		StartFrameMoving(lib, self:GetParent())
	end
	local function onDragStop(self)
		if IsFrameMoving(lib, self:GetParent()) then
			StopFrameMoving(lib)
		end
	end
	--[[
...
function IsFrameGroup(self, frame, group)
	return GetFrameGroup(lib, frame) == group or group == true
end
...
function InFrameGroup(self, frame, group)
	return group == frame or (type(group) == "table" and group[frame])
end
