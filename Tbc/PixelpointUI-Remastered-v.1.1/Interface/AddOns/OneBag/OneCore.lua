--$Id: OneCore.lua 45043 2007-07-28 03:16:37Z kaelten $
...
function OneCore:OnInitialize()
    ...
    self:Hook("GetContainerNumSlots", function(id)
        ...
        if id == -2 then
            local numKeyringSlots = self.hooks.GetContainerNumSlots(KEYRING_CONTAINER);
...
function module:BuildFrame()
	...
	if self.isBank then
		if not self.frame.bags[BANK_CONTAINER] then 
			...
			self.frame.bags[BANK_CONTAINER].size = GetContainerNumSlots(BANK_CONTAINER)
...
function module:OrganizeFrame(needs)
	...
	local cols, curCol, curRow, justinc = profile.cols, 1, 1, false
	
	self.soulSlots, self.ammoSlots, self.profSlots, self.slotCount, self.totalCount = 0, 0, 0, 0, 0
	
	for k, bag in pairs(self.fBags) do 
		if bags[bag] then
			for k2, v2 in ipairs(bags[bag]) do 
				v2:Hide()
			end
            self.totalCount = self.totalCount + (bags[bag].size or 0)
        end
	end
	
	local startCol = 1
    if profile.vAlign == L["Bottom"] then
        curCol = math.fmod(self.totalCount, cols) > 0 and cols - math.fmod(self.totalCount, cols) + 1 or 1
        if profile.bagBreak then
            curCol = math.fmod(bags[0] and bags[0].size,cols) > 0 and cols - math.fmod(bags[0] and bags[0].size,cols) + 1 or 1
        end
    end
    
    
	for k, bag in pairs(profile.show.direction and self.fBags or self.rBags) do
		local curBag = bags[bag]
        
		if curBag and curBag.size and curBag.size > 0 then
            if profile.bagBreak and bag > 0 and math.fmod(bags[bag-1] and bags[bag-1].size or 0, cols) ~= 0 then 
				if (profile.vAlign == L["Bottom"]) then
					startCol = math.fmod(curBag.size, cols) > 0 and cols - math.fmod(curBag.size, cols) + 1 or 1
					curRow = curRow - 1 
				end
                curCol, curRow = startCol, curRow + 1
            end
