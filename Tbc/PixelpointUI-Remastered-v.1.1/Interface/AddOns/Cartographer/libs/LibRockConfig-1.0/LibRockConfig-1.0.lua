...
local MINOR_VERSION = tonumber(("$Revision: 65058 $"):match("(%d+)")) - 60000
...
do
	...
	do
		...
		local function editBox_OnMultilineTextChanged(this, ...)
			local parent = this:GetParent()
			local text = this:GetText()
			-- resize parent to number of lines in text
			local _, n = text:gsub("\n","\n")
			n = n * 14 + 28
		...
		local function saveButton_OnClick(this)
			...
			if parent.value == editBox:GetText() then
				editBox:ClearFocus()
				return
			end
			if parent.validationIssue then
			...
			local tmp = newList(editBox:GetText())
			callConfigField(parent, 'set', tmp)
			tmp = del(tmp)
