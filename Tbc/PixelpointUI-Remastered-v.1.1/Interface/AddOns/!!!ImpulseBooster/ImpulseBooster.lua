-- Name: ImpulseBooster
-- Author: Onotole @ Feenix Archangel, a WoW 2.4.3 Private Server
-- License: LGPL v2.1
...
local match = string.match or function(s,p) local _,_,m = find(s,p) return m end
...
--
UseCVars({"coresDetected", "gxRefresh", "gxResolution", "gxVSync", "maxFPS", "maxFPSBk", "timingMethod", "timingTestError", "processAffinityMask"})
...
local DISPLAY_IBSYNC = not DISPLAY_VSYNC and DISPLAY_RATES[DISPLAY_MAX_FPS] ~= nil
...
local function OnEvent()
	...
	elseif event == "PLAYER_LOGIN" then
		...
		local stats = format("UI startup: %2.2fs", abs(TIME_LOADING_END - TIME_LOADING_START))
...
BOOSTER:SetScript("OnUpdate", OnUpdate)
--
do -- Inject Video Options
	local optionsvideo = _G["OptionsFrameDisplay"]
	local attachto = _G["OptionsFrameCheckButton"..((WOW < 20000 and 7) or 14)] -- Hardware cursor checkbox for TBC clients, LOD checkbox for Vanilla clients
