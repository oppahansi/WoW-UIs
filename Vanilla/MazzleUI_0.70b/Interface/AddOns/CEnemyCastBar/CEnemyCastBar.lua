...
Info for Burning Crusade version:
...
Change gcinfo use collectgarbage("count"); lines prepared for testing, s. other textfile for new GC commands (evtl. "setstepmul" auf 10000 + "collect" beim öffnen, beim schließen wieder auf 200 setzen :D)
...
Many new Events/Functions will be added to recognize spell casting of Units!
]]--

CECB_status_version_txt = "5.3.9";
...
function CEnemyCastBar_RegisterEvents(unregister)
	...
	if (playersclass ~= "ROGUE") then
		CarniEnemyCastBarFrame:UnregisterEvent("PLAYER_COMBO_POINTS");
	end
end
...
function CEnemyCastBar_OnEvent(event) --onevent
	...
	elseif ( event == "VARIABLES_LOADED" ) then
		...
		-- my addons support, used for helpframe, too!
			...
			CEnemyCastBarHelp[2] = "|cff00ff00/necb versions |cffffffffor |cff00ff00gversions |cffffffff- Group/GuildMembers NECB versions.";
			CEnemyCastBarHelp[2] = CEnemyCastBarHelp[2].."\n".."|cff00ff00/necb parser |cffffffff- Opens the NECB - AddOn Channel parser.";
			CEnemyCastBarHelp[2] = CEnemyCastBarHelp[2].."\n".."|cff00ff00/necb chat Text |cffffffff- Sends 'Text' to other NECB's in your group.";
...
function CEnemyCastBar_Handler(msg) --Handler
	...
	elseif (string.sub (msg, 1, 5) == "cast " and CEnemyCastBar.bStatus) then
		...
		-- for BC use this macro and remove two lines above!:
...
function CEnemyCastBar_Player_Target_Changed() --Target changed
	...
	while i <= table.getn(NECB_CD_DB) do
		if ( GetTime() > ( NECB_CD_DB[i][3] + NECB_CD_DB[i][4] ) ) then
			...
			if (NECB_CD_DB[i][1] == UnitName("target") and UnitIsPlayer("target") and not CEnemyCastBar_Spells[NECB_CD_DB[i][2]].disabled) then
...
function CEnemyCastBar_Control(mob, spell, special, bcasted, event) --Control
	...
	if (
		...
		if (CEnemyCastBar.bPvP) then
			...
			-- CD Buffer Routine
			if (CEnemyCastBar.bUseCDDB and CEnemyCastBar.bCDown and CEnemyCastBar_Spells[spell] and CEnemyCastBar_Spells[spell].d ) then
...
function CECB_GCInfo_OnUpdate() --gcinfo
	...
	if (not lastgcupdate or GCTime - lastgcupdate > 0.5) then
		...
		--local cecbgc_now = collectgarbage("count"); -- after BC, delete above!

		if (not cecbgc_last or cecbgc_last > cecbgc_now) then
			cecbgc_last = cecbgc_now;
			cecbgc_min = cecbgc_now;
			--cecbgc_max = cecbgc_now*2; -- after BC and use global
		...
		if (not lastgcdiffupdate or GCTime - lastgcdiffupdate > 5) then
			...
			-- for BC, clear from here downwards, remove time presumption
