...
function oRAOMainTank:GetPosition( f )
	...
	if self.db.profile.layout == L["Horizontal"] then -- HORIZONTAL POSITIONING
	  ...
	  -- adjust positioning for mtt if mt is shown
		if f.type == "mtt" and self.db.profile.showmt then
			relframe = self.mtf[f:GetID()]
			relx = MazzleUI_Settings.oRA.width
...
function oRAOMainTank:SetStyle(f)
	...
	self:SetWHP(f, MazzleUI_Settings.oRA.width, 21, "TOPLEFT", relframe, "TOPLEFT", relx , rely)
	...
	self:SetWHP(f.bar, (MazzleUI_Settings.oRA.width - 8), 16, "LEFT", f, "LEFT", 4, 0)
	self:SetWHP(f.bar.texture, (MazzleUI_Settings.oRA.width - 8), 16, "CENTER", f.bar, "CENTER", 0, 0)
	...
	self:SetWHP(f.raidicon, 14, 14, "LEFT", f.bar, "LEFT", 0, MazzleUI_Settings.oRA.raidIconIndent)
	--self:SetWHP(f.name, 62, 14, "LEFT", f.bar, "LEFT", 18, 0)
	self:SetWHP(f.name, MazzleUI_Settings.oRA.nameWidth, 14, "LEFT", f.bar, "LEFT", MazzleUI_Settings.oRA.nameIndent, 0)
	self:SetWHP(f.bar.text, 36, 14, "RIGHT", f.bar, "RIGHT", 0, 0)
