...
SA_GC_HELP=false; -- WILL cause problems, only for testing!
...
function SA_List_Update()
	...
	-- clear some tables, reduces amount of GC needed
	if (SA_GC_HELP) then
		local mem = gcinfo();
		SA_ClearTable(candidates);
		SA_ClearTable(targets);
		SA_Debug("SA_GC_HELP freed "..tostring(math.abs(gcinfo() - mem)), 5);
	end
end
