...
-- $LastChangedDate: 2006-08-27 00:38:48 +0300 (su, 27 elo 2006) $
-- $LastChangedBy: marko $
-- $Rev: 254 $
...
SmartAssistDetails = {
		...
		version = "$3",
		releaseDate = string.sub("$LastChangedDate: 2006-08-27 00:38:48 +0300 (su, 27 elo 2006) $", 18, 28),
		author = "paranoidi",
...
SMARTASSIST_CORE_REV = tonumber(string.sub("$Rev: 254 $", 7, -3));
...
function SA_AggroedTargetMsg()
	...
	-- todo: add as option!
	if (SCT) then
		SCT:DisplayCustomEvent("Target engaged!", COLOR_DEFAULT, true, false, 1);
	end
end
...
function FindTarget(candidates, allowNearest, recursive)
	...
	if (SA_TableSize(PREVIOUS_ASSISTS)>0 and not recursive) then
		...
		PREVIOUS_ASSISTS = {};
		return FindTarget(candidates, members, allowNearest, true);
	end
