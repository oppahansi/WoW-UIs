...
-- Bartender support?
-- Named Slots
-- Initialization error on first startup -> AceDB
...
function AutoBar_BuildItemList()
...
function AutoBar:ButtonsUpdate()
	...
	for buttonnum = 1, AUTOBAR_MAXBUTTONS, 1 do
		button = _G["AutoBarFrameButton"..buttonnum];
		icon = _G["AutoBarFrameButton"..buttonnum.."Icon"];
		normalTexture = _G["AutoBarFrameButton"..buttonnum.."NormalTexture"];
		countText = _G["AutoBarFrameButton"..buttonnum.."Count"];
		hotKey = _G["AutoBarFrameButton"..buttonnum.."HotKey"];
		cooldown = _G["AutoBarFrameButton"..buttonnum.."Cooldown"];
...
function AutoBar:AssignButtons()
	...
	for buttonIndex = 1, AUTOBAR_MAXBUTTONS, 1 do
		if (AutoBar.display.showEmptyButtons or AutoBar_Buttons_CurrentItems[buttonIndex]) then
			...
			local button = _G["AutoBarFrameButton"..displayButton];
	...
	for buttonIndex = displayButton+1, AUTOBAR_MAXBUTTONS, 1 do
		local button = _G["AutoBarFrameButton"..buttonIndex];
...
function AutoBar_UpdateCategoryNameToID(name,id)
...
function AutoBar_Button_OnClick(mousebutton, updown, override)
	...
	if (override) then
		button = _G["AutoBarFrameButton"..override];
...
-- It is goddam retarded but getn doesnt.
function maxn(t)
	local n = 0;
...
function AutoBar_SetupVisual()
	...
	local i;
	local anchor = _G["AutoBarAnchorFrameHandle"];
	for i = 1, AUTOBAR_MAXBUTTONS, 1 do
		local button = _G["AutoBarFrameButton"..i];
	...
	_G["AutoBarAnchorFrameHandle"]:SetChecked(AutoBar.display.frameLocked);
...
function AutoBar:UpdatePopupButtons(baseButton)
	...
	local autoBarPopup = _G["AutoBarPopupFrame"];
	local gapping = AutoBar.display.gapping;
	local buttonWidth = AutoBar.display.buttonWidth;
	local buttonHeight = AutoBar.display.buttonHeight;
	...
	for popupButtonIndex = 1, AUTOBAR_MAXPOPUPBUTTONS, 1 do
		button = _G["AutoBarPopupFrame_Button"..popupButtonIndex];
		button:Hide();
	end
	...
	for popupButtonIndex, currentItems in pairs(AutoBar_Buttons_CurrentItems[AutoBarPopupFrame.popupButton]) do
		...
		category = AutoBar_SearchedForItems[itemId][2];
		button = _G["AutoBarPopupFrame_Button"..popupButtonIndex];
		hotKey = _G["AutoBarPopupFrame_Button"..popupButtonIndex.."HotKey"];
		icon = _G["AutoBarPopupFrame_Button"..popupButtonIndex.."Icon"];
		countText = _G["AutoBarPopupFrame_Button"..popupButtonIndex.."Count"];
		normalTexture = _G["AutoBarPopupFrame_Button"..popupButtonIndex.."NormalTexture"];
		cooldown = _G["AutoBarPopupFrame_Button"..popupButtonIndex.."Cooldown"];
			...
			if (buttonInfo.arrangeOnUse) then
				...
				for index = 1, AUTOBAR_MAXSLOTCATEGORIES, 1 do
					if (buttonInfo[index] == category) then
						...
						if (AutoBar_Category_Info[category].arrangeOnUse) then
...
--DEFAULT_CHAT_FRAME:AddMessage("arrangeOnUse bad category")
							AutoBar.ConfigChanged();
						end
						break;
					end
...
function AutoBar:ShowHandle(frame)
	local handle = _G[frame:GetName().."Handle"];
	if (handle) then
		handle:SetScale(0.6);
		handle:Show();
	end
...
-- TODO: Mmm, a complicated nightmare of settings.  Perhaps just link known bindings to named slots like say MOUNT
function AutoBar:UpdateHotkeys()
	for i = 1, AUTOBAR_MAXBUTTONS, 1 do
		local key = AutoBar.buttons[i]["key"];
		if (key) then
			local binding = _G["AUTOBAR_BUTTON"..i];
