...
Recap_MinorVersion = 3	-- Minor version (Rophy's work)
...
function Recap_DamageEvent(info)

	local i, source, dest, damage, miss, effect, crit
	local found = false
	local blocked = false

	if recap.Opt.State.value~="Stopped" then

		if info.type ~= "hit" and info.type ~= "miss" and info.type ~= "cast" then return end
		
		blocked = info.amountBlock

		source = info.source
		if source == ParserLib_SELF then source = recap_temp.Player end
		
		dest = info.victim
		if dest == ParserLib_SELF then dest = recap_temp.Player end
		...
		if effect == ParserLib_MELEE then effect = "Melee" end
		if effect == ParserLib_DAMAGESHIELD then effect = "Damage Shields" end
		...
		if damage then -- if this wasn't just a cast
			...
			if info.type == "cast" and (not recap.Opt.LightData.value) and recap.Combatant[source] and recap.Combatant[source].Detail["1"..effect] then
		...
		if damage and damage>0 then
			...
			if not recap.Opt.LightData.value then
				if info.skill == ParserLib_MELEE then -- melee damage
...
function Recap_DeathEvent(info)
	...
	if recap.Opt.State.value~="Stopped" then
		...
		if info.type ~= "death" then return end
		
				victim = info.victim
				
				if info.source or info.isItem then return end

				if victim == ParserLib_SELF then victim = recap_temp.Player end
...
function Recap_HealEvent(info)
	...
	if recap.Opt.State.value~="Stopped" then
		...
		if info.type ~= "heal" then return end
				...
				if not recap.Opt.LightData.value then
					Recap_CreateEffect(source,effect,3,heal,crit)
...
function Recap_Combat_OnEvent(e, a1)
	if not e then e = event end
	if not a1 then a1 = arg1 end

	if e=="CHAT_MSG_COMBAT_FRIENDLY_DEATH" or e=="CHAT_MSG_COMBAT_HOSTILE_DEATH" then
		Recap_DeathEvent(a1)
	...
	elseif e=="SPELLCAST_START" then
		if (not recap.Opt.LightData.value) and recap.Combatant[recap_temp.Player] and recap.Combatant[recap_temp.Player].Detail["1"..a1] then
			if not recap.Opt.LimitFights.value then
				if recap.Opt.IdleFight.value then
					recap_temp.IdleTimer = 0
				end
	...
	elseif string.find(e,"BUFF") then
		Recap_HealEvent(a1)
	else
		Recap_DamageEvent(a1)
	end
end
...
function Recap_Register_CombatEvents()
	...
	-- register damage events
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_CREATURE_VS_CREATURE_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_CREATURE_VS_CREATURE_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_CREATURE_VS_PARTY_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_CREATURE_VS_SELF_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_CREATURE_VS_SELF_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_FRIENDLYPLAYER_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_FRIENDLYPLAYER_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_HOSTILEPLAYER_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_HOSTILEPLAYER_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_PARTY_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_PARTY_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_CREATURE_VS_PARTY_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_PET_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_PET_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_SELF_HITS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_SELF_MISSES", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_CREATURE_VS_PARTY_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_CREATURE_VS_SELF_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_DAMAGESHIELDS_ON_OTHERS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_HOSTILEPLAYER_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PARTY_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_HOSTILEPLAYER_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_PARTY_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PET_DAMAGE", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_SELF_DAMAGE", Recap_Combat_OnEvent)

		-- register death events
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_FRIENDLY_DEATH", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_COMBAT_HOSTILE_DEATH", Recap_Combat_OnEvent)

		-- register healing events
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_CREATURE_VS_CREATURE_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_CREATURE_VS_PARTY_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_CREATURE_VS_SELF_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_FRIENDLYPLAYER_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_HOSTILEPLAYER_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PARTY_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_BUFFS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_HOSTILEPLAYER_BUFFS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_PARTY_BUFFS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_PET_BUFF", Recap_Combat_OnEvent)
		parser:RegisterEvent(addonID, "CHAT_MSG_SPELL_SELF_BUFF", Recap_Combat_OnEvent)
...
function Recap_Unregister_CombatEvents()
	if recap_temp.CombatEventsRegistered then
		...
		parser:UnregisterAllEvents(addonID);
