...
Change Log:
v1.4
-TOC updated to 11200
-Added TradeSkillSkillIcon linking
...
function AH_QuickSearch_SetUpHooks()
	...
	if (TradeSkillSkillIcon) then
		AH_QuickSearch_SetUpTradeSkillHooks();
	else
		-- Hook them later
		local oldFunc = TradeSkillFrame_LoadUI;
		TradeSkillFrame_LoadUI = function() oldFunc(); AH_QuickSearch_SetUpTradeSkillHooks(); end
	end
...
function AH_QuickSearch_SetUpTradeSkillHooks()
	-- Hooks TradeSkillSkillIcon
	local oldFunc = TradeSkillSkillIcon:GetScript("OnClick");
	if (oldFunc) then
		TradeSkillSkillIcon:SetScript("OnClick", function() oldFunc(); AH_QuickSearch_ItemButton_OnClick(arg1); end);
	else
		TradeSkillSkillIcon:SetScript("OnClick", function() AH_QuickSearch_ItemButton_OnClick(arg1); end);
	end
	--Hook TradeSkillReagents
	for i=1, 8 do
		local frame = getglobal("TradeSkillReagent"..i);
		local oldFunc = frame:GetScript("OnClick");
		frame:SetScript("OnClick", function() oldFunc(); AH_QuickSearch_ItemButton_OnClick(arg1); end);
	end
end
