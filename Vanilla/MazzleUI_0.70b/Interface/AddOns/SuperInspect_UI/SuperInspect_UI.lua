...
function SuperInspect_OnLoad()
	...
	--copied from outfitter, thanks :S
	SI_RunningWoW2 = GetCritChance ~= nil;
	
	if not SI_RunningWoW2 then
...
function SuperInspect_OnEvent()
	...
	if( event == "PLAYER_TARGET_CHANGED") then
		...
		if (SuperInspectFrame:IsVisible() or SI_Save.targ) then
			...
			SuperInspect_InspectUnit(UnitName("target"));
	...
	if ( event == "UNIT_INVENTORY_CHANGED" ) then
		if ( arg1 == "target" ) then
			...
			SI_Save.itemscached = false;
			SuperInspect_UpdateTargetCache();
			if (SuperInspect.current_unit) then
				SuperInspect_ApplyItemsToFrames(SuperInspect.current_unit);
			end
		end
		return;
	end
...
function SuperInspect_GetLinkForButton(unit,button)
		return GetInventoryItemLink(unit, button:GetID()); 
end

function SuperInspect_UpdateTargetCache()
	local unittoload = "target";
	--if target not present recache the player
	if (not UnitExists(unittoload)) then
		unittoload = "player";
	end
	if (UnitExists(unittoload)) then
		if (UnitIsPlayer(unittoload) and UnitIsFriend("player",unittoload)) then
			--SI_AddMessage('found a target, caching its infos for later use.');
			local unitname = UnitName(unittoload);
			if (not SI_LOCAL_USERS_ITEM_CACHE[unitname]) then
				SI_LOCAL_USERS_ITEM_CACHE[unitname] = {};
			end
			if (not SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos'] ) then
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos'] = {};
			end		
			local uname,urealm = UnitName(unittoload);	
			if ((CheckInteractDistance(unittoload, 1))) then
				if (not SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'] ) then
					SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'] = {};
				end			
				SI_Save.itemscached = true;
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['cache_date'] = time();
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectHeadSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectHeadSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectNeckSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectNeckSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectShoulderSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectShoulderSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectBackSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectBackSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectChestSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectChestSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectShirtSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectShirtSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectTabardSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectTabardSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectWristSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectWristSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectHandsSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectHandsSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectWaistSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectWaistSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectLegsSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectLegsSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectFeetSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectFeetSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectFinger0Slot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectFinger0Slot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectFinger1Slot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectFinger1Slot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectTrinket0Slot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectTrinket0Slot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectTrinket1Slot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectTrinket1Slot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectMainHandSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectMainHandSlot); 
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectSecondaryHandSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectSecondaryHandSlot); 				
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][SuperInspect_InspectRangedSlot] = SuperInspect_GetLinkForButton(unittoload,SuperInspect_InspectRangedSlot); 								
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['realm'] = urealm;
			end
			local pvpname = UnitPVPName(unittoload);
			if (pvpname) then
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['pvpname'] = pvpname; 
			end
			local guildname = nil;
			local guildtitle = nil;
			local guildrank = nil;	
			guildname, guildtitle, guildrank = GetGuildInfo(unittoload);
			if (guildname) then
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['guildname'] = guildname;
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['guildtitle'] = guildtitle;
				SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['guildrank'] = guildrank;					
			end
			SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['race'] = UnitRace(unittoload);
			SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['class'] = UnitClass(unittoload);
			local level = UnitLevel(unittoload);
			if (level == -1) then
				level = "??";	
			end		
			SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['level'] = level;	
			NotifyInspect(unittoload);	
		end
	end
end

function SuperInspect_InspectUnit(unitname)
	if (not unitname) then
		return;
	end
	SI_AddMessage('inspect user:'..unitname);
	SuperInspect.current_unit = unitname;
	SuperInspect.name = unitname;
	
	--ok now try to load itemdata of the target player!
	SuperInspect_UpdateTargetCache();
	SuperInspect_ApplyModel(unitname);
	SuperInspect_ApplyItemsToFrames(unitname);
	SuperInspect_ApplyInfosToFrames(unitname);
	SuperInspect_UpdateButtons(unitname);
	SuperInspect_UpdateCachedList();
end

function SuperInspect_ApplyInfosToFrames(unitname)
	if (SI_LOCAL_USERS_ITEM_CACHE[unitname]) then
		SuperInspectFrameHeader_Info:SetText(SI_LEVEL.." "..SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['level'].." "..SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['race'].." "..SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['class']);				
		if (SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['guildname']) then
			SuperInspectFrameHeader_Guild:SetText(format(TEXT(GUILD_TITLE_TEMPLATE), SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['guildtitle'], "<"..SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['guildname']..">"));
		else
			SuperInspectFrameHeader_Guild:SetText(" ");				
		end
		if (SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['pvpname']) then
			SuperInspectFrameHeader_Name:SetText (SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['pvpname']);
		else
			SuperInspectFrameHeader_Name:SetText (unitname);
		end
		if (SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['realm']) then
			SuperInspectFrameHeader_Realm:SetText (SI_LOCAL_USERS_ITEM_CACHE[unitname]['infos']['realm']);
		else
		...
		if (SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'] ) then
			...
			SuperInspect_InRangeFrame_Text2:SetText(SI_TEXT_CACHEPLAYERRANGE);		
		end
		if ((UnitName("target") == unitname) and (UnitName("player") == unitname) ) then
		...
		if (( UnitExists("target")) and UnitName("target") == unitname) then	
			if (UnitRace("target")) then
				...
				if (not pvpname) then
					SuperInspectFrameHeader_Name:SetText(UnitName("target"));
				else
					SuperInspectFrameHeader_Name:SetText(UnitPVPName("target"));
				end		
				local guildname = nil;
				local guildtitle = nil;
				local guildrank = nil;	
				...
				SuperInspectFrameHeader_Info:SetText(SI_LEVEL.." "..level.." "..UnitRace("target").." "..UnitClass("target"));								
				if (guildname) then
				...
				SuperInspectFrameHeader_Info:SetText(SI_LEVEL.." "..level..classification);
				SuperInspectFrameHeader_Guild:SetText(" ");
				SuperInspectFrameHeader_Realm:SetText(" ");				
			end
		end
...
function SuperInspect_UpdateButtons(unitname)
	if (( UnitExists("target")) and UnitName("target") == unitname) then
...
function SuperInspect_ApplyItemsToFrames(unitname)
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectHeadSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectNeckSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectShoulderSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectBackSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectChestSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectShirtSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectTabardSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectWristSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectHandsSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectWaistSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectLegsSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectFeetSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectFinger0Slot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectFinger1Slot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectTrinket0Slot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectTrinket1Slot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectMainHandSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectSecondaryHandSlot);
	SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,SuperInspect_InspectRangedSlot);
...
function SuperInspect_ApplyModel(unitname)
	SuperInspect_ModelFrame:ClearModel();
	SuperInspect_SetBackground();
	local unittoload = "target";
	--if target not present recache the player
	...
	if (( UnitExists(unittoload)) and UnitName(unittoload) == unitname) then
		SuperInspect_ModelFrame:SetModelScale(1);
		SuperInspect_ModelFrame:ClearModel();
		SuperInspect_ModelFrame:SetUnit(unittoload);

		SuperInspect_ModelFrame:SetPosition(x, y, z);

		--DEFAULT_CHAT_FRAME:AddMessage(SuperInspect_ModelFrame:GetModelScale());
		SetPortraitTexture(SuperInspectFramePortrait,unittoload);
		...
		SuperInspect_SetBackground(unittoload);
	end
...
function SuperInspect_InspectFrame_Show(unit)
	...
	if ((unit == "SI_FORCE") or (( UnitExists(unit) and SuperInspect.player))) then
		...
		--SuperInspect_ShowwonorFrame.isShowing) then
			SuperInspect_HonorFrame:Hide();
		else
			SuperInspect_HonorFrame:Show();
		end
...
function SuperInspect_Main_OnUpdate()
	if ( SuperInspect.isVisible) then
		if ( CheckInteractDistance("target", 1) and SI_Save.itemscached == false) then 
			SuperInspect_UpdateTargetCache();

			if (SuperInspect.current_unit) then
				SuperInspect_ApplyItemsToFrames(SuperInspect.current_unit);
				SuperInspect_ApplyInfosToFrames(SuperInspect.current_unit);
			end
			SuperInspect_UpdateCachedList();			
		end
		if ( not CheckInteractDistance("target", 1) and not SuperInspect_HonorFrame.isShowing ) then
			if (SuperInspect_Button_ShowHonor.isVisible ) then
			...
			SuperInspect_UpdateButtons(SuperInspect.current_unit);
...
function SuperInspect_UpdateCachedList()
	...
	local sorthlp = {};
	for index, value in pairs(SI_LOCAL_USERS_ITEM_CACHE) do
			if (SI_LOCAL_USERS_ITEM_CACHE[index]['items']) then
				countmax = countmax + 1;
				sorthlp[countmax] = index;
			end
	end
	table.sort(sorthlp);
	...
	for n = 1, countmax do
			...
			if (SI_LOCAL_USERS_ITEM_CACHE[index]['items']) then
				...
				if (((lineplusoffset + 1) <= i) and ((lineplusoffset + 1 + maxitems) > i)) then
					...
					local color = SI_HudClass[SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['class']];
					txt:SetText(index);
					txt:SetTextColor(color[1],color[2],color[3]);
					local newtxt = "";
					newtxt = SI_LEVEL.." "..SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['level'].." "..SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['race'].." "..SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['class'];
					txt = getglobal("SIInfoPlayer"..internalcount.."Info");
					txt:SetText(newtxt);	
					txt = getglobal("SIInfoPlayer"..internalcount.."Realm");
					if (not (SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['realm'] == " ")) and (SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['realm']) then
						txt:SetText('<'..SI_LOCAL_USERS_ITEM_CACHE[index]['infos']['realm']..'>');	
					else
						txt:SetText(" ");
					end
					frm = getglobal("SIInfoPlayer"..internalcount);
					frm:SetBackdropColor(1,1, 1, 0);
					if (index == SuperInspect.current_unit) then
...
function SuperInspect_InspectPaperDollItemSlotButton_Update(unitname,button)
	...
	if (SI_LOCAL_USERS_ITEM_CACHE[unitname]) then
		...
		if (SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'] ) then
			...
			if (SI_LOCAL_USERS_ITEM_CACHE[unitname]['items'][button]) then
				...
				local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = GetItemInfo(mylink);
				if (not SI_RunningWoW2) then
					itemName, itemLink, itemQuality, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = GetItemInfo(mylink);
				end
	...
	if (link) then
			...
			local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = GetItemInfo(itemCode);
			if (not SI_RunningWoW2) then
				itemName, itemLink, itemQuality, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = GetItemInfo(itemCode);
			end
			--SuperInspectTTScan:SetOwner(SuperInspect_InspectPaperDollFrame, "ANCHOR_NONE");
			SuperInspectTTScan:ClearLines();
			SuperInspect_TT:ClearLines();		
			if (( UnitExists("target")) and (UnitName("target") == unitname)  and (UnitName("player") == unitname) and UnitIsUnit("target", "player")) then	
				SuperInspectTTScan:SetInventoryItem("player", button:GetID());
				SuperInspect_TT:SetInventoryItem("player", button:GetID());			
			else
	...
	if ((not SuperInspect.bonus or SuperInspect.bonus ~= unitname)) then
	...
	-- ok fine load from cache works!
	button.link = nil;
	if (link) then
		--SI_AddMessage('link found!'..link);
		...
		if ((nstart) and (nstart == 1) and (nend == 5)) then
			...
			if (not SI_RunningWoW2) then
				itemName, itemLink, itemQuality, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = GetItemInfo(link);
			end
			local r, g, b, hex = GetItemQualityColor(itemQuality);
			button.link = link;
			...
			if (SI_RunningWoW2) then
				for color, item in string.gmatch(link, "|c(%x+)|Hitem:(%d+:%d+:%d+:%d+:%d+:%d+:%d+:%d+)|h%[.-%]|h|r") do
					--SI_AddMessage("test:"..item);
					if( item and item ~= "" ) then
						button.link = "item:"..string.gsub(item, "^(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+)$", "%1:%2:%3:%4:%5:%6:%7:%8");
...
function SuperInspectItemBonuses_ScanLine(line, id)
	...
	elseif(string.sub(line,0,string.len(SI_IB_COH_PREFIX)) == SI_IB_COH_PREFIX) then
		...
		if (SuperInspect_ItemBonuses_coh) then
			local fnd = nil;
			local j = 1;
			for index, value in pairs(SuperInspect_ItemBonuses_coh) do
				if (index == item) then
	...
	elseif(string.sub(line,0,string.len(SI_IB_USE_PREFIX)) == SI_IB_USE_PREFIX) then
		...
		if (SuperInspect_ItemBonuses_use) then
			local fnd = nil;
			local j = 1;
			for index, value in pairs(SuperInspect_ItemBonuses_use) do
				if (index == item) then
...
function SuperInspectItemBonuses_ScanPassive(line)
	...
	found = nil;
	for i,p in pairs(SI_IB_EQUIP_PATTERNS) do
...
function SuperInspectItemBonuses_ScanOther(line)
	...
	found = nil;
	for i,p in pairs(SI_IB_OTHER_PATTERNS) do
...
function SuperInspectItemBonuses_ScanToken(token, value)
	...
	if(SI_IB_TOKEN_EFFECT[token]) then
		...
		s1 = nil;
		s2 = nil;
		for i,p in pairs(SI_IB_S1) do
			if(string.find(token,p.pattern,1,1)) then
				s1 = p.effect;
			end
		end	
		for i,p in pairs(SI_IB_S2) do
			if(string.find(token,p.pattern,1,1)) then
				s2 = p.effect;
			end
		end	
...
function SuperInspectItemBonusesButton_BuildTooltip()
	...
	for i,e in pairs(SI_IB_EFFECTS) do
	...
	if (text and text ~= "") then
		...
		if (SuperInspect_ItemBonuses_sets and table.getn( SuperInspect_ItemBonuses_sets ) >= 1) then
			...
			for i=1, table.getn( SuperInspect_ItemBonuses_sets ), 1 do
				...
				if (SuperInspect_ItemBonuses_sets[i]["t"]) then
					...
					for index, value in pairs(SuperInspect_ItemBonuses_sets[i]["t"]) do
		...
		for index, value in pairs(SuperInspect_ItemBonuses_coh) do
			cohText = cohText.."\n\n".. SI_IB_YELLOW..index;
			for index2, value2 in pairs(SuperInspect_ItemBonuses_coh[index]) do
		...
		for index, value in pairs(SuperInspect_ItemBonuses_use) do
			useText = useText.."\n\n".. SI_IB_YELLOW..index;
			for index2, value2 in pairs(SuperInspect_ItemBonuses_use[index]) do
				useText = useText.."\n ".. SI_INFOTEXT_COLOR..value2;	
				--ChatFrame4:AddMessage(index.." : "..index2.."="..value2);
				bonusTab = true;
				useTab = true;
...
function SuperInspect_HonorFrame_OnShow()
	...
	if (SI_RunningWoW2) then
		-- This week's values
		SuperInspect_HonorFrameThisWeekTitle:Hide();
		SuperInspect_HonorFrameThisWeekHK:Hide();
		SuperInspect_HonorFrameThisWeekContribution:Hide();
				
		-- Last Week's values
		SuperInspect_HonorFrameLastWeekTitle:Hide();
		SuperInspect_HonorFrameLastWeekHK:Hide();
		SuperInspect_HonorFrameLastWeekContribution:Hide();
		SuperInspect_HonorFrameLastWeekStanding:Hide();	
		
		SuperInspect_HonorFrameLifeTimeRank:Hide();	
		
		SuperInspect_HonorFrameYesterdayContributionText:SetText(HONOR_POINTS);
		SuperInspect_HonorFrameLifeTimeDKText:SetText(HONOR_POINTS);
		SuperInspect_HonorFrameLifeTimeDK.tooltip = NEWBIE_TOOLTIP_HONOR_STANDING;		
		SuperInspect_HonorFrameCurrentDKText:SetText(HONOR_POINTS);
		SuperInspect_HonorFrameCurrentDK.tooltip = NEWBIE_TOOLTIP_HONOR_STANDING;		
	end	
...
function SuperInspect_HonorFrame_Update()
	...
	if (SI_RunningWoW2) then
		...
		local sessionHK, sessionHonor,yesterdayHK, yesterdayHonor, lifetimeHK, CurrentHonor = GetInspectHonorData();
		...
		SuperInspect_HonorFrameLifeTimeDKValue:SetText("0");	

				
		-- This session's values
		SuperInspect_HonorFrameCurrentHKValue:SetText(sessionHK);
		SuperInspect_HonorFrameCurrentDKValue:SetText(sessionHonor);

		
		local factionGroup = UnitFactionGroup("target");
		if ( factionGroup ) then
			SuperInspect_HonorFramePvPIcon:SetTexture("Interface\\TargetingFrame\\UI-PVP-"..factionGroup);
			SuperInspect_HonorFramePvPIcon:Show();	
		else
			SuperInspect_HonorFramePvPIcon:Hide();	
		end
		SuperInspect_HonorFrameCurrentPVPRank:SetText(CurrentHonor);
		...
		SuperInspect_HonorFrameProgressBar:SetValue(100);
		
		SuperInspect_HonorFrameCurrentPVPTitle:SetPoint("TOP", "$parent", "TOP", - (SuperInspect_HonorFrameCurrentPVPRank:GetWidth())/2, -30);	
...
function SuperInspect_InspectPaperDollItemSlotButton_OnEnter()
	...
	local loadfromlink = true;
	if (UnitName("player") == SuperInspect.current_unit) then
		loadfromlink = not GameTooltip:SetInventoryItem("player", this:GetID());
	end
	
	if (loadfromlink) then
		if (this.link) then
