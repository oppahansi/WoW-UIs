...
TheoryCraft_TooltipFormat = {
	...
	{show = "embedstyle3", 	left = "#c0.9,0.9,1##OR$dps$#c1,1,1# DPS/$hpm,2$#c1,1,1# HpmOR#", 
			       right = "#c0.9,0.9,1#$critchance,1$%#c1,1,1# Kritisch"},
	{show = true, 		left = "#c1,1,1#$basecasttime$", right = "#c1,1,1#$spellcooldown$"},
	...
	{show = true, 		left = "ungebufft: $sealunbuffed$"},
	{show = true, 		left = "mit diesem Siegel: $sealbuffed$"},
	...
	{show = "nextpen", 	left = "+10 pen: Up to +$nextpendam,2$ Ø $hitorheal$#IF (Entspricht: $nextpendamequive,2$ $damorap$)IF#"},
	...
	{show = "lifetap", 	left = "Aderlass DPH: $lifetapdpm$"},
	{show = "lifetap", 	left = "Aderlass HPH: $lifetaphpm$"},
	{show = "lifetap", 	left = "Aderlass DPS: $lifetapdps$"},
	{show = "lifetap", 	left = "Aderlass HPS: $lifetaphps$"},
	...
	{show = "max", 		left = "Max Heilung: $maxoomheal$"},
	{show = "maxtime", 	left = "Max Heilung: $maxoomheal$ ($maxoomhealtime$ Sek)"},
	{show = "max", 		left = "Max Schaden: $maxoomdam$"},
	{show = "maxtime", 	left = "Max Schaden: $maxoomdam$ ($maxoomdamtime$ Sek)"},
	{show = "maxevoc", 	left = "Max Schaden mit evoc+gem: $maxevocoomdam$"},
	{show = "maxevoctime", 	left = "Max Schaden mit evoc+gem: $maxevocoomdam$ ($maxevocoomdamtime$ Sek)"},
}
...
TheoryCraft_MeleeMinMaxReader = {
	...
	{ pattern = "plus (%d+)",												-- Schreddern/Verheeren/Hinterhalt/Maucheln
		type={"addeddamage"} },
	{ pattern = " um (%d+)",												-- Maul/Zermalmen/Gezielter Schuß/Heldenhafter Stoß
		type={"addeddamage"} },
...
-- warlock
	...
	{ pattern = "(%d+) Feuerschaden und nahen Gegner (%d+)(.+)",						-- Höllenfeuer
		type={"bothdamage", "bothdamage", "tmptext"} },
	{ pattern = "Seele und verursacht (.+) lang (%d+)",									-- Seelendieb
		type={"tmptext", "dotbothdamage"} },
	{ pattern = "fügt (.+) Sek. lang (%d+) Punkt",										-- Fluch der Pein
		type={"tmptext", "dotbothdamage"} },
-- mage
	{ pattern = "(%d+) bis (%d+)(.+)zusätzlich 12 Sek. lang (%d+) bis (%d+)",			-- Pyroblast
		type={"mindamage", "maxdamage", "tmptext", "dotmindamage", "dotmaxdamage"} },		
	{ pattern = "(%d+) bis (%d+)(.+) und zusätzlich 8 Sek. lang (%d+) bis (%d+)",		-- Flame Shock
		type={"mindamage", "maxdamage", "tmptext", "dotmindamage", "dotmaxdamage"} },
	{ pattern = "(.+) pro Sekunde (%d+) bis (%d+)",										-- Arkane Geschosse
		type={"tmptext", "dotmindamage", "dotmaxdamage"} },
	{ pattern = "(.+) pro Sekunde (%d+)",												-- Arkane Geschosse
		type={"tmptext", "dotbothdamage"} },
	{ pattern = "(%d+) bis (%d+) Punkte%(e%) Feuerschaden (.+) Sek. lang (%d+)",		-- Feuerball, Flammenstoß, Feuerbrand (WL)
		type={"mindamage", "maxdamage", "tmptext", "dotbothdamage"} },
	{ pattern = "der (%d+) bis (%d+) Punkte%(e%) Frostschaden",							-- Frostblitz
		type={"mindamage", "maxdamage"} },
-- hunter
	{ pattern = "sofortiger Schuss, der (%d+)",											-- Arkaner Schuss
		type={"bothdamage"} },
-- more
	{ pattern = "verursacht (%d+) bis (%d+) Feuerschaden bei sich selbst und (%d+) bis (%d+) Feuerschaden",	-- Hellfire
	...
	{ pattern = "Furcht für 3 Sek und verursacht (%d+) Schattenschaden",				-- Death Coil
		type={"bothdamage"} },
	{ pattern = "(%d+) bis (%d+)(.+)und weitere (%d+) bis (%d+)",						-- Generic Hybrid spell
		type={"mindamage", "maxdamage", "tmptext", "dotmindamage", "dotmaxdamage"} },
	{ pattern = "(%d+) bis (%d+)(.+)und weitere(%d+)",									-- Generic Hybrid spell
		type={"mindamage", "maxdamage", "tmptext", "dotbothdamage"} },
	{ pattern = "(%d+) bis (%d+)(.+)zusätzlich (%d+) bis (%d+)",						-- Generic Hybrid spell
		type={"mindamage", "maxdamage", "tmptext", "dotmindamage", "dotmaxdamage"} },
	{ pattern = "(%d+) bis (%d+)(.+)zusätzlich (%d+)",									-- Generic Hybrid spell
		type={"mindamage", "maxdamage", "tmptext", "dotbothdamage"} },
	{ pattern = "(%d+) bis (%d+)(.+) und (%d+) bis (%d+)",								-- Generic Hybrid spell
		type={"mindamage", "maxdamage", "tmptext", "dotmindamage", "dotmaxdamage"} },
...
TheoryCraft_Locale = {
	...
	SpellTranslator = {
		...
		["Drain Soul"] = "Lebensentzug",
		["Siphon Life"] = "Seelendieb",
...
TheoryCraft_CheckButtons = {
	...
	["averagedamnocrit"] = {short = "Ohne Krit", description = "If checked crits won't be included in the Average Hit value." },
	...
	["dontcritdpm"] = { hide = {"ROGUE", "WARRIOR"}, short = "Ohne Krit", description = "If checked crits won't be included in the DPM value." },
	["hpm"] = { hide = {"ROGUE", "WARRIOR"}, short = "HPM", description = "Average Heal divided by True Mana Cost" },
	["dontcrithpm"] = { hide = {"ROGUE", "WARRIOR"}, short = "Ohne Krit", description = "If checked crits won't be included in the HPM value." },
	["showregendam"] = { hide = {"ROGUE", "WARRIOR"}, short = "Regenerieren Schaden", description = "Shows how much extra damage you could do\ngiven 10 seconds of normal regen, compared to\n10 seconds of regen whilst casting" },
	["showregenheal"] = { hide = {"ROGUE", "WARRIOR"}, short = "Regenerieren Heilung", description = "Shows how much extra healing you could do\ngiven 10 seconds of normal regen, compared to\n10 seconds of regen whilst casting" },
	["max"] = { hide = {"ROGUE", "WARRIOR"}, short = "Max bis oom", description = "Shows how much damage/healing you can do before going oom,\nchaincasting the spell including all normal forms of regen and\nthe settings for dpm (regarding crits/resists)." },
	...
	["maxtime"] 	= { hide = {"ROGUE", "WARRIOR"}, short = "Zeit bis oom", description = "Adds how long it takes to go oom, chain casting that spell." },
	["lifetap"] 	= { hide = {"ROGUE", "WARRIOR", "MAGE", "SHAMAN", "HUNTER", "DRUID", "PRIEST"}, short = "Aderlass Werte", description = "DPS, DPM, HPS, HPM if enabled will have\nadditional info for if you're using Lifetap.\nTakes in to account the global cooldown." },
	["embedstyle1"]	= { short = "DPS | Krit", description = "Adds an extra line in the middle of the tooltip,\nwith DPS/HPS on the left and Crit chance on the right.", descriptionmelee="For melee, will only show your crit chance above\nthe description of each ability." },
	["embedstyle2"]	= { hide = {"ROGUE", "WARRIOR"}, short = "DPM | Krit", description = "Adds an extra line in the middle of the tooltip,\nwith DPM/HPM on the left and Crit chance on the right." },
	["embedstyle3"]	= { hide = {"ROGUE", "WARRIOR"}, short = "DPS/HPM | Krit", description = "Adds an extra line in the middle of the tooltip,\nwith DPS/HPM on the left and Crit chance on the right." },
	...
	["tryfirstlarge"] = { short = " ", tooltiptitle = "Großer Text", description = "Centers the text and makes it much larger.\nMostly useful if you limit button text to a select\nnumber of spells, eg low rank heals." },
	...
	["trysecondlarge"] = { short = " ", tooltiptitle = "Großer Text", description = "Centers the text and makes it much larger.\nMostly useful if you limit button text to a select\nnumber of spells, eg low rank heals." },
	["framebyframe"] = { short = "Frame für Frame", tooltiptitle = "Begrenzung auf einen Button pro Frame", description = "Forces button text to be generated one button per frame, instead of all at once.\nNormally this is only done only in combat, as each button is virtually instant to\ngenerate.  On very slow computers you may wish to force TC to always generate\nbuttons this way, by ticking this checkbox." },
	...
	["mitigation"] = { short = "Rüstung einbeziehen", description = "If enabled your targets armor will be included in TC's calculations.\nYou can view a mobs armor by typing in /tc armor 'mob name', or\njust leaving it blank to list all known mobs." },
...
-- needs translation
	...
	{ text="Increases critical chance of spells by (%d+)%%,", type="Allcritchance" },					-- Onyxia
...
--checking needed
	...
	{ text="%+(%d+) Schaden und Heilungszauber", type="All" },						-- of sorcery items
	{ text="Heilungszauber %+(%d+)", type="Healing" },								-- zg priest and healing enchant
	{ text="%+(%d+) Zauberschaden und Heilung", type="All" }, 						-- not sure
	{ text="Benutzen: Stellt 375 bis 625 Punkt%(e%)Mana wieder her%.", type="manarestore", amount="500" },    -- Robe of the Archmage
