...
--		Print(Counter)
		...
		elseif buttontext.type == "Bonus" then
			if ( this.isBonus and CURRENT_ACTIONBAR_PAGE==1 ) then
				...
				TCTooltip:SetAction(this:GetID() + ((NUM_ACTIONBAR_PAGES+offset-1)*NUM_ACTIONBAR_BUTTONS))
				spelldata = TheoryCraft_GetSpellDataByFrame(TCTooltip)
			else
...
function TheoryCraft_AddButtonText()
	...
	local function setupbutton(parentname, type, specialid)
	...
	if ActionButton1 then
		for i = 1,12 do setupbutton("ActionButton"..i, "Normal") end
		...
		for i = 1,10 do setupbutton("BonusActionButton"..i, "Bonus") end
	end
end