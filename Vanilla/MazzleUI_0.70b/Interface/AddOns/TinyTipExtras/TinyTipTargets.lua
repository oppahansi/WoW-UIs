...
function TinyTipTargets_Mouse(unit,flag)
	...
	if _G.UnitExists(pid) then
		...
		tmp2 = strformat("%s : |cFF%s%s|r", 
				...
				(not tmp and (_G.UnitName(pid) or TinyTipTargetsLocale_Unknown)) or _G.TinyTipTargetsLocale_YOU)
		if flag == 2 then
			fsSetText( c, tmp2)
		else
...
function TinyTipTargets_Raid(unit,num,flag)
		...
		if flag == 2 then
			...
			for c,tmp in pairs(RaidTable) do
		...
		for i = 1,num,1 do
			...
			if _G.UnitIsUnit(pid .. "target",unit) and not _G.UnitIsUnit(pid,"player") then
				...
				elseif flag == 3 then
					tmp2 = strformat("%s|cFF%s%s|r\n", 
										  tmp2 or "", _G.TinyTip_ColorPlayer(pid), _G.UnitName(pid) or TinyTipTargetsLocale_Unknown)
					c = c + 1
				else
		...
		elseif tmp2 and tmp2 > 0 then
			if ( GetLocale() == "koKR" ) then
				gtAddLine( tooltip, strformat("%s: |cFFFFFFFF%s|r명의 %s원", 
...
function TinyTipTargets_Party(unit, num, flag)
		...
		if tmp2 then
			if flag ~= 2 then
				...
				if ( GetLocale() == "koKR" ) then
					gtAddLine( tooltip, strformat("%s: |cFFFFFFFF%s|r명의 %s", 
