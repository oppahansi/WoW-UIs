...
function CML_SlashCommand(msg)
	...
	elseif(command == "off") then
		...
		ReloadUI();					-- To unload all Hooked Stuff   // got many problems so this fixed all, sorry
...
function CML_Reset() 
	CML_Vars = {};
	CML_Vars.Version = 1.10;			-- Version Number for Auto reset
	CML_Vars.Enabled = true;			-- default enabled
	CML_Vars.Debug = false;				-- default disabled 
	CML_Vars.PostRandom = true;			-- default enabled 	-- Message on Randomloot
	CML_Vars.Ask = false;				-- default disabled
	CML_Vars.Quickloot = "";			-- Playername for Quickloot
	CML_Vars.ShowRolls = 5;				-- only show Top 5 Rolls
	CML_Vars.RollStartTime = 0;			-- Starttime since Anouncement
	CML_Vars.RollTimeout = 60;			-- Only Accept Rolls within X Sec
	CML_Vars.Rolls = {};				-- empty Array for Roll's
	CML_Vars.Roll_Min = 1;				-- Default 1
	CML_Vars.Roll_Max = 100;			-- Default 100

	CML_Vars.BidBodSupport = false;			-- LV BidBod Support
	CML_Vars.BidBodExpr = "%[BidBot%]: (%d+). (%w+) hat (%d+) DKP geboten.";	-- Expression to Target Result from
	CML_Vars.BidBodStartString = "!bid %s";		-- String to start Bids
	CML_Vars.BidBodBids = {};			-- empty Array for Bids
end
...
function CML_InitDropDown()
	...
	if( CML_Vars.Enabled == false ) then		-- Addon is OFF - Open the Default Menu
		CML_Text("Now run original DropDown handler!");
		CML_ORGGROUPLOOTDROPDOWN();
		return true;
	end
	
	if( GetNumRaidMembers() > 0) then		-- in Raidgroup
		local raidmembers = {};
		local maxrolls = 0;
		...
		if(UIDROPDOWNMENU_MENU_LEVEL == 2) then				-- Layer 2 in List

			if( UIDROPDOWNMENU_MENU_VALUE == "ROLLLIST" ) then	-- Build the Roll Menue

				maxrolls = table.getn(CML_Vars.Rolls);		-- only show TopX Rolls
...
--								info.disabled = 1;
							...
							info.text =  GetMasterLootCandidate(i).." - "
								   ...
								   ..date("%H:%M:%S %d.%m.%y", CML_Vars.BidBodBids[x].time).." - "
								   ..infoname;
							info.value = i;
							info.notCheckable = 1;
							info.func = CML_GiveItTo;
							UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);												end
					end
				end

			-- Normal List
			else							-- Normal ClassLoot Level2
				if( raidmembers[UIDROPDOWNMENU_MENU_VALUE] == nil ) then return false; end -- I don't know why, but otherwise for is bugged
		...
		elseif(UIDROPDOWNMENU_MENU_LEVEL == 1) then					-- Layer 1 on List
			...
			if( notinquickloot == false ) then
				UIDropDownMenu_AddButton{
					text = "Quickloot: "..CML_Vars.Quickloot,
					value = quicklootid,
					func = CML_GiveItTo,
				}
			end
...
function CML_AnnounceRoll()
	...
	local msg = "";
	CML_Vars.Rolls = {};			-- Reset Roll Table
	CML_Vars.RollStartTime = time();
...
function CML_GiveToRandomTarget()		-- only works in Raids 
	local list_players = {}
	local candidate = "";
	local winner = 0;
	...
	while( list_players[winner]["lootid"] == 0 ) do			-- This Loop is need because a Player can't be on the LootTable
	...
	if( CML_Vars.PostRandom == true ) then			-- Show Roll Messages?
...
function CML_Hook()
	CloseLoot();			-- First Close a opened LootFrame
	CML_HideDropDownMenu();		-- Second Close all Dropdown Menues

	-- Hook the Loot Dropdown Menue
	if( CML_ORGGROUPLOOTDROPDOWN == nil ) then							-- Only need if not the first time
		CML_ORGGROUPLOOTDROPDOWN = GroupLootDropDown_Initialize;				-- Save old Lootmenu
	end
--	GroupLootDropDown_Initialize = CML_InitDropDown;						-- Init own Lootmenu
	UIDropDownMenu_Initialize(GroupLootDropDown, CML_InitDropDown, "MENU");				-- Init own Dropdown on Loot

	-- Hook the GiveMasterLoot Function
	if( CML_ORGGIVEMASTERLOOT == nil ) then								-- Only need if not the first time
...
function CML_OnEvent(event)
	if( event == "VARIABLES_LOADED" ) then		-- hmm PLAYER_LOGIN seems to be better
	
		if( CML_Vars == nil or CML_Vars.Version == nil or not(CML_Vars.Version == 1.10) ) then
			CML_Reset();
		end
		
		if( CML_Vars.Enabled ) then		-- If addon is not enabled, we don't need to Hook the Functions
			CML_Hook(); 			-- Hooking Functions
		else
			CML_Text( CML_ISDISABLED );	-- Information that the Addon is Disabled
		end
	...
	if( event == "CHAT_MSG_SYSTEM" ) then		-- All /rolls comes there
		if( time() <= (CML_Vars.RollStartTime + CML_Vars.RollTimeout) ) then				-- wan't lesser CPU Usage?
...
function CML_ShowRollTable()					-- prints out the Roll Result Table
	local msg = "";
	local maxrolls = table.getn(CML_Vars.Rolls);		-- only show TopX Rolls
	if( maxrolls > CML_Vars.ShowRolls ) then 
		maxrolls = CML_Vars.ShowRolls; 
	end
...
function CML_CheckRoll(player)				-- returns true if player allready rolled
	for i=1, table.getn(CML_Vars.Rolls) do
		if( not (CML_Vars.Rolls[i] == nil) ) then
			if( CML_Vars.Rolls[i].name == player ) then
				return true;
			end
			CML_Text("CML_CheckRoll ID: "..i.." - "..CML_Vars.Rolls[i].name.."("..CML_Vars.Rolls[i].roll..")");	-- Debug
		end
	end
	return false;
end

function CML_SetRoll(player, roll)			-- Insert the Player and Roll in the Rolls Table
	local TheRoll = {};
	TheRoll["name"] = player;
...
function CML_GetGroupType()
	if( GetNumRaidMembers() > 0) then		-- in Raid?
		return "RAID";
	else						-- in Party?
		return "PARTY";
	end
