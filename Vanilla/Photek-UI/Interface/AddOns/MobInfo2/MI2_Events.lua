...
local function MI2_VariablesLoaded()
	...
	for school, schortcut in MI2_SpellSchools do
...
local function MI2_OnTargetChanged()
	...
	if MI2_DebugEvents > 0 then if MI2_Target then chattext( "M2DBG: new target: idx=["..(MI2_Target.index or "nil").."], last=["..(MI2_LastTargetIdx or "nil").."]" )
...
--if mob then chattext( "DBG: SPELLIMMUNESELFOTHER: spell="..spell ) end
						if not mob then
...
local function MI2_EventSpellPeriodic_ZHTW()
	...
	local s,e, dummy1, dummy2, damage = string.find(arg1, MI2_ChatScanStrings[9])
	if damage then
		MI2_RecordHit( tonumber(damage) )
	end
end -- MI2_EventSpellPeriodic()
...
local function MI2_EventSelfBuff()
	...
	if lootAction and lootType then
		if MI2_DebugEvents > 0 then chattext("M2DBG: non mob loot event: action="..lootAction..", type="..lootType ) end
...
local function MI2_EventHostileDeath()
	...
	if creatureName then
		if MI2_DebugEvents > 0 then chattext("M2DBG: no XP kill event: mob="..creatureName ) end
...
function MI2_EventCreatureDiesXP()
	local s,e, creatureName, xp = string.find( arg1, MI2_ChatScanStrings[3] )
	if creatureName and xp then
		if MI2_DebugEvents > 0 then chattext("M2DBG: kill event with XP: mob="..creatureName..", xp="..xp ) end
		MI2_RecordKill( creatureName, tonumber(xp) )
	end
...
local function MI2_EventZoneChanged()
	...
	if MI2_DebugEvents > 0 then chattext("M2DBG: zone change event: zone="..zone..", cont="..continent ) end
...
function MI2_InitializeEventTable()
	...
	for eventName, eventInfo in MI2_EventHandlers do
...
eventInfo.on = eventEnabled -- ONLY FOR DEBUGGING, TO BE DELETED
...
if MI2_DebugEvents > 0 then
local e=MI2_EventHandlers[event]
if not e or not e.f or not e.on then
--chattext( "DBG: ["..event.."] received but not enabled/supported" )
end end
...
function MI2_OnLoad()
	...
	MI2_ChatScanStrings = {
		[1] = OPEN_LOCK_SELF,
		[2] = SELFKILLOTHER,
		[3] = COMBATLOG_XPGAIN_EXHAUSTION1,
	...
	for idx, scanString in MI2_ChatScanStrings do
...
--chattext( "DBG: processing scan str "..idx..": org=["..MI2_ChatScanStrings[idx].."], new=["..scanString.."]")
